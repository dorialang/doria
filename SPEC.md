...
Namespace/file-scope import `use` is implemented and resolved at compile time. Class-body or trait-body trait-composition `uses` preserves ordered generic references and `insteadof`/`as` adaptations in Slice 1; application to the composer remains a Slice 4 boundary.
