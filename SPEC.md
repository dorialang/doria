...
`Displayable` retains its narrow compiler-known concrete runtime path while general interface values remain deferred to Stage 35 Slice 2:
...
Interface values/storage/conversions/type tests stop before HIR with E0758
(Slice 2); new core-contract operations and public iteration use E0759
(Slice 3); trait composition uses E0493 (Slice 4). Invalid declarations and
primitive interface erasure receive language diagnostics, not a future-feature
excuse. None of these declarations introduces a runtime object or interface ABI.
