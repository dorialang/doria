...
The native backend is the primary target. Checked HIR lowers to scalar-typed MIR, Cranelift lowers MIR directly to object code, and the host linker combines that object with `doria-rt`. Native compilation has no interpreter preflight or fallback IR. Generated functions maintain hidden runtime stack-frame metadata. `doria-rt` owns entry policy, exact stdout/stderr writes, abort-only panic formatting, stack traversal, and status 101. Cranelift selects width/signedness-correct integer instructions, `F32`/`F64` IEEE operations without fast-math, canonical `I8` bool ABI values, short-circuit control flow, and explicit numeric conversion checks. Runtime failures branch to the shared Doria panic path instead of exposing backend traps. Only canonical int/void entry results cross the process boundary. Compile-time-known string expressions still lower as exact bytes without defining a runtime string ABI. Unsupported coverage remains for runtime strings, string parameters/returns, collections, classes/methods, labeled loop control, LLVM, and broader later-stage features.
...
- LLVM release code generation over the Stage 14 MIR scalar model.
- Broader native runtime capabilities beyond the current Stage 14 scalar and compile-time-string subset.
