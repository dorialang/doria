...
The current Stage 11f MIR is that growing native-oriented, backend-independent control-flow representation for a narrow subset. It contains multiple top-level free functions with deterministic IDs, parameter/local slots, basic blocks, branch and jump terminators, typed bool condition trees, integer comparison operands, and `int`/`void` call forms. The debug target interprets this MIR directly using isolated call frames, shared stdout, a global bounded execution budget, exact-state cycle detection, and a defensive call-depth limit. Stage 11f supports helpers with `int` parameters and `int`/`void` returns, positional calls, Stage 11c `if` control flow, structured `while`, traditional `for`, integer range `foreach`, nested loop control, integer mutation, early return, and exact string-literal `echo` in void functions. Helper `int` returns are full Doria `int` values; only `main(): int` crosses the portable process-status boundary. Stage 11f rejects direct and mutual recursion before interpretation and does not introduce truthiness, user-authored bool runtime locals, collection iteration, methods/static calls, constructors, classes, string parameters/returns, bool helper returns, runtime strings, ownership/borrow checking, doria-rt, LLVM, NativeSmokeModule deletion, or complete Stage 11 coverage.
