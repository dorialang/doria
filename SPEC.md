...
The current Stage 11g MIR is that growing native-oriented, backend-independent control-flow representation for a narrow subset. It contains deterministic top-level functions, typed int/string local slots, basic blocks, typed conditions, and int/void call forms. Stage 11g adds readonly inferred/explicit string locals and string expressions composed of literals, readonly local reads, and ordered `.` concatenation for `echo` in void functions, including supported helpers, branches, and loops. The debug interpreter uses isolated typed frames, shared exact-byte stdout with no implicit newline, a global bounded execution budget, exact-state cycle detection, and a defensive call-depth limit. Helper `int` returns remain full Doria `int` values; only `main(): int` crosses the process-status boundary. Interpreter-private Rust strings do not define Doria heap allocation, ownership, layout, termination, or runtime concatenation semantics. Stage 11g still excludes writable string locals, string assignment, string parameters/returns, display conversion, collection iteration, methods/static calls, constructors, classes, recursion, ownership/borrow checking, doria-rt, Cranelift-from-MIR, LLVM, NativeSmokeModule deletion, and complete Stage 11 coverage.
