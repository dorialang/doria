...
The current compiler implementation lowers the accepted native subset through validated typed MIR. The debug interpreter, default Cranelift fast profile, and `--release` LLVM profile consume that same MIR, and the durable executable parity manifest compares exact stdout bytes, stderr bytes, and process status across all three paths. The supported subset has top-level free functions plus exactly one parameterless `function main(): int`/`int64` or `function main(): void`; path-sensitive returns; recursion; all eight fixed-width integer types; IEEE binary32 and binary64 floats; runtime bool values; scalar locals, parameters, returns, and calls; contextual numeric literals; integer and float arithmetic/comparison; bool value expressions and short-circuiting; explicit checked integer conversions; `Int::toFloat` and `Float::toInt`; supported structured control flow; and compile-time-known readonly string expressions for exact-byte `echo` and fatal `panic`. `main(): int` crosses the accepted `0..125` process boundary, while helper scalar values retain their declared semantics. `main(): void` maps normal completion to status `0`. Release optimization does not change overflow, panic, float, bool, string, output, or process-status semantics. The allocation-free bootstrap `doria-rt` owns process entry, exact native output, panic formatting, and Doria function-name stack traces. Runtime strings, string parameters/returns, writable runtime strings, collection iteration, methods/classes, ownership/borrow checking, and broader runtime features remain unsupported. The former Stage 7-10 native smoke module remains retired.
...
The `.` operator is string concatenation. Both operands must be `string` values or recovery types; Doria does not implicitly convert `int`, `bool`, objects, or other values to `string` for concatenation. The current MIR/interpreter/Cranelift/LLVM subset supports `.` only when the resulting string value is compile-time-known through supported string literals and readonly string locals.
...
Native is the primary target. Checked HIR lowers to scalar-typed MIR, shared MIR validation gates both native lowerers, Cranelift emits the default fast object, LLVM 18 emits the O3 `--release` object, and the host linker combines either object with `doria-rt`. Native compilation has no interpreter preflight, fallback IR, or release-to-fast fallback. Generated functions maintain hidden runtime stack-frame metadata. `doria-rt` owns entry policy, exact stdout/stderr writes, abort-only panic formatting, stack traversal, and status 101. Both lowerers select width/signedness-correct operations, binary32/binary64 IEEE operations without fast-math, canonical bool ABI values, short-circuit control flow, and explicit numeric conversion checks. Runtime failures branch to the shared Doria panic path instead of exposing backend traps or LLVM undefined behavior. Only canonical int/void entry results cross the process boundary. Compile-time-known string expressions still lower as exact bytes without defining a runtime string ABI. Unsupported coverage remains for runtime strings, string parameters/returns, collections, classes/methods, labeled loop control, and broader later-stage features.
...
- Runtime strings and canonical display conversion beyond the current scalar and compile-time-string subset.
