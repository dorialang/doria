...
- Stage 30e native closure execution is complete through the debug interpreter, Cranelift fast profile, and LLVM release profile. Keep closure descriptors static and lean, use the two-word descriptor/environment carrier, allocate no environment for no-capture closures, use stack storage for proven nonescaping environments, and use one heap allocation for escaping environments. Acquire captures in source order and release remaining owned captures in reverse logical order. Ordinary closures must not use reference counting. `E0641` is now PHP-only and names Stage 30f. Do not reintroduce it into checking, IDE analysis, HIR, MIR, debug, or native execution, and do not claim Stage 30 complete.
