...
- Stage 11 is MIR + interpreter oracle. Stage 11a seeds MIR, Stage 11b adds int expressions and locals, Stage 11c adds typed conditions and `if`, Stage 11d adds `while` and loop control, Stage 11e adds `for` and integer range `foreach`, Stage 11f adds top-level free functions and bounded call frames, and Stage 11g adds readonly string locals plus literal/local/`.` echo expressions with exact stdout bytes and no implicit newline. Stage 11g does not define runtime strings and still excludes writable string locals, string parameters/returns, display conversion, collection iteration, methods/static calls, constructors, classes, recursion, ownership/borrow checking, doria-rt, Cranelift-from-MIR, LLVM, and NativeSmokeModule deletion. Track Stage <=10 parity in `docs/notes/stage-11-mir-parity-matrix.md`; new language capability work should go through MIR/interpreter rather than NativeSmokeModule.
