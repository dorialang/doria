...
- Stages 11 through 13 are complete. The debug interpreter and Cranelift consume the same typed MIR; the durable manifest at `crates/doriac/tests/fixtures/native_parity_examples.txt` drives exact stdout/stderr/status differential tests and must cover every finite `examples/native/*.doria` fixture. General control flow, recursion, all eight fixed-width integer types, contextual integer literals, integer operators, and explicit checked conversions share that path. `doria-rt` owns native entry, exact output, and abort-only panic behavior. Stage 14 is next: float execution and bool runtime values. Do not recreate a parallel native IR, bypass MIR, or execute user code as a native-compilation preflight.
...
- Treat current native string-expression `echo` as a compile-time-known exact-byte path only. It writes no implicit newline and must not lower through newline-adding helpers such as `puts`; it does not define runtime strings or display conversion.
- Do not claim runtime strings, interpolation, formatted output, `Console`, or general runtime I/O are implemented because compile-time-known native `echo` and panic messages work. Stages 12 and 13 retain the same exact-byte output and status-101 panic foundation.
- Treat panic as fatal, non-catchable, and non-unwinding. Explicit `panic("message")`, every Stage 13 checked-integer failure, and an invalid `main(): int` process status use the abort-only status-101 path defined by decisions 0040, 0041, 0042, and 0044.
...
When native backend work changes supported behavior, run both linker-independent MIR-to-Cranelift object tests and the complete durable manifest-driven interpreter/Cranelift differential suite. The current Stage 13 target supports top-level free functions, one parameterless `main`, path-sensitive returns, recursion and mutual recursion, general supported nested control flow, every fixed-width signed and unsigned integer type, contextual literals, checked arithmetic, division/remainder, shifts, bitwise operations, unary negation, increments/compound assignments, and checked companion `from` conversions. It also retains compile-time-known readonly string-expression echo and abort-only panic. Normal interpretation has no artificial block or call-depth cap, and native compilation has no interpreter preflight. Integer helper values retain their full declared Doria range; only `main(): int` crosses the `0..125` process-status boundary. The durable manifest must include every finite native example and compare exact stdout, stderr, and status, including Stage 13 panic fixtures. Do not treat the internal `doria-rt` ABI as stable or claim runtime floats, runtime bool values, runtime strings, classes, collections, ownership, LLVM, or broader Stage 14+ support.
