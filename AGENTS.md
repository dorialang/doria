...
- Stages 11 through 19 are complete. The debug interpreter, Cranelift fast profile, and LLVM release profile consume the same validated typed MIR; the durable manifest at `crates/doriac/tests/fixtures/native_parity_examples.txt` drives exact stdin/stdout/stderr/status, file-side-effect, and class-lifetime differential tests and must cover every finite `examples/native/*.doria` fixture. General control flow, recursion, fixed-width numerics, runtime bool and string values, narrow `?string`, checked formatting, UTF-8 text I/O, full primitive expression interpolation, and Stage 19 native class construction, ownership transfer, properties, lifecycle bodies, and deterministic normal-exit destruction share that path. `doria-rt` owns native entry, class allocation/free, raw device I/O, line discipline, file I/O, exact output, canonical display conversion, refcounted strings, and abort-only panic behavior. Stage 20 methods, statics, and `internal` native lowering is next. Do not recreate a parallel native IR, bypass shared MIR validation, or execute user code as a native-compilation preflight.
...
- `Displayable` is a compiler-known nominal contract requiring explicit `implements Displayable` and exactly `function toString(): string`. Do not accept `__toString`, structural conformance, or general interfaces early. Frontend/PHP support exists; native class execution remains Stages 19/20 and general interfaces remain Stage 35.
