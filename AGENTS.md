...
- Stage 30d closure HIR/MIR and debug-interpreter execution are complete. Keep closure descriptors static and lean, use the logical two-word descriptor/environment carrier, allocate no environment for no-capture closures, acquire captures in source order, and release owned captures in reverse logical order. Ordinary closures must not use reference counting. `E0641` is target-specific: native execution remains Stage 30e and PHP lowering remains Stage 30f. Do not reintroduce `E0641` into checking, IDE analysis, HIR, MIR, or debug execution, and do not claim Stage 30 complete.
