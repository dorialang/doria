...
- `doria-language-server` is a separate repository that is **in scope for compiler work**, unlike `doria-website`, and pins `doriac` by git revision in its root `Cargo.toml`. Never infer where its checkout lives; developers and automation provide its path explicitly. Because it restates language facts in hover text, diagnostics, and highlighting, it goes stale the moment a stage lands — and a stale language server tells the user their valid code is wrong, which is worse than no language server. "Coordinate" means **complete the required work with the tooling agent in the same beat and verify the result**, not merely mention it in a report or take over that agent's work. See "Language-server sweep".
- Andrew is the language designer and sole developer. He reviews and approves before anything advances.

### Agent coordination

The compiler agent owns compiler work. Existing specialist agents own tooling,
website issues, and debugging/experimentation. Coordinate with those agents when
work overlaps: identify the existing task and agree ownership before editing
shared files. Exchange concrete repros, affected contracts, commit identities,
and validation evidence. Preserve each other's work and avoid duplicate fixes.
Notify the website agent of relevant compiler changes without taking over its
repository. A handoff is not proof of completion; verify required integration
before reporting the compiler beat complete.
...
A delivered stage, slice, or beat is not complete while developer-facing binaries still represent the previous compiler. Run this during active development and again after the compiler and coordinated language-server changes are committed:
...
The refresh installs compiled `doriac` and `doria-lsp` artifacts into Cargo's platform-neutral install root, verifies that both embed the same compiler revision, and rejects `PATH` entries that shadow them. A clean compiler checkout uses its exact Git commit. An active dirty checkout uses a deterministic `dev-<sha256>` identity over the current tracked and untracked source tree, so editor semantics and every compiler-revision-keyed Baton, processor, and language-server cache change whenever the source being tested changes; it never masquerades as the clean `HEAD` commit. The language-server checkout may also be dirty and is rebuilt from its current source. Release artifacts remain clean-commit builds. Its installation build target lives under the managed validation target (or `DORIA_VALIDATION_TARGET_DIR`), but it is a reclaimable build cache rather than an installed component: the refresh deletes that target before verification, then requires the installed compiler to compile and execute a native smoke program. Standalone `doriac` embeds both deterministic `doria-rt` profiles and materializes the selected verified archive outside Cargo's target tree; an installed compiler must never retain an absolute dependency on `target/`. Editor clients and Baton must consume these installed artifacts (or explicit/bundled release artifacts), never an implicitly discovered workspace `target/debug` executable.
...
- Built-in free-function names are fully worded and never fused: `read_line`, not `readline`. Whitelisted abbreviations are only those more recognizable than their expansions, plus industry-universal single lexemes such as `printf`. Whitelisting is always explicit and documented in the plan's naming charter.
- Keep type-coupled vocabulary on the type companion. For strings, `$string->` is limited to intrinsic data, measurements, and views, while every string-specific operation is a camelCase `String::` companion member. Doria has no public `str_*` family or string-operation instance-method aliases.
- String text operations use one shared Unicode implementation across the MIR interpreter and `doria-rt`: `length`, search indices, replacement, splitting, slicing, and padding operate on extended grapheme-cluster boundaries; `byteLength` alone reports UTF-8 bytes. Backends lower validated String intrinsics to the runtime ABI and never substitute host-locale or platform string behavior.
...
- Do not treat `array` as a Doria type. Doria has C-style typed arrays spelled `T[]`, such as `int[] $numbers`; broader collection APIs use `List<T>`, `Dictionary<K, V>`, `SortedDictionary<K, V>`, `Set<T>`, `SortedSet<T>`, `PriorityQueue<T>`, and `Deque<T>`. `Deque<T>` covers queue and stack use cases; `Queue<T>` and `Stack<T>` are not Doria type names. `array $items` and `List<array>` are invalid Doria surface syntax. PHP backend output may still use PHP `array` internally when lowering Doria collections.
...
- Keep closure execution backend-independent. Native closure descriptors are static and lean, use the two-word descriptor/environment carrier, allocate no environment for no-capture closures, use stack storage for proven nonescaping environments, and use one heap allocation for escaping environments. PHP consumes semantic and validated MIR closure plans through explicit compiler-generated carriers, environments, and `BindingId`-backed stable places; PHP automatic capture, `use (...)`, host callable semantics, and PHP references never define Doria behavior. Acquire captures in source order and release remaining owned captures in reverse logical order. Ordinary closures do not use reference counting.
...
- Treat basic `if` / `else if` / `else`, `while`, base `do ... while`, traditional `for`, integer range `foreach`, standalone `++` / `--`, and unlabeled `break;` / `continue;` as executable control flow. `if` is statement control flow and does not return a value; `when` is the executable value-returning conditional expression. `given` may prepare and gate `if`, `when`, or `while`; its setup runs once, while predicates on `given ... while` reevaluate before every condition check. Control-flow `finally` executes on `if`, `when`, `while`, and `do ... while` through shared backend-neutral finalizer regions. Normal completion and crossed structured exits run each finalizer exactly once, nested regions run inner-to-outer, same-loop `continue` skips that loop's finalizer, and fatal panic runs no finalizer. `for` is the explicit counter/index loop; `foreach` is preferred for collections and ranges. `0..10` is inclusive, `0..<10` is exclusive-end, and range `foreach` bindings are readonly per iteration. `break` exits the nearest enclosing loop, and `continue` jumps to the next nearest-loop iteration.
- Every `foreach` binding must carry an explicit type; `writable` is an access modifier, not a substitute for one. In two-binding `foreach`, treat the first source binding as syntax-neutral until semantics classifies it. `List<T>` and `T[]` provide a readonly zero-based canonical-`int` sequence index; Dictionary families provide their actual readonly key. Ranges, sets, deques, and Dictionary projections remain value-only. Compiler inference may power a missing-type diagnostic and fix, but it never accepts omitted binding types. Do not infer this matrix again in HIR, MIR, a backend, or tooling.
...
- Interpolation, string-anchored concatenation, and `%s` materialize an ordinary reusable `string`; they do not authorize direct scalar-to-string assignment, argument coercion, primitive `toString`, `String::from` aliases, or casts.
...
- Every revision this repository pins into another repository is declared in `cross-repo-pins.json` and verified by `scripts/check_cross_repo_pins.php`, which proves the revision is reachable from a published ref rather than merely present in a local checkout. Declare a new pin there; do not write another bespoke checker. Never restate a revision in the manifest — the manifest says where the revision lives and how to read it, so there is exactly one copy. `scripts/check_benchmark_revision.php` validates the pinned sibling *checkout* and is not a substitute: a sibling on unpushed work satisfies it while the pin names a commit nobody else can fetch. Sibling repositories carry their own `cross-repo-pins.json` and are verified with `--siblings`. Private repositories require an explicit read-only credential: CI uses `DORIA_CROSS_REPO_TOKEN` when configured and requires remote verification in that case; without it, reachability is reported as `UNVERIFIED`, never as passed. Publishing or coordinated release work must run `--require-remote` with suitable credentials.
- Compiler performance-report structure is opt-in and derived from existing AST, semantic, HIR, MIR, validation, code-generation, runtime-selection, and link work. Do not add an extra counting pass or make ordinary compilation allocate report state. Missing resource/profiler metrics are unavailable, never zero.
