...
- Stages 11 through 18 are complete. The debug interpreter, Cranelift fast profile, and LLVM release profile consume the same validated typed MIR; the durable manifest at `crates/doriac/tests/fixtures/native_parity_examples.txt` drives exact stdin/stdout/stderr/status and file-side-effect differential tests and must cover every finite `examples/native/*.doria` fixture. General control flow, recursion, fixed-width numerics, runtime bool and string values, narrow `?string`, checked formatting, UTF-8 text I/O, and full primitive expression interpolation share that path. `doria-rt` owns native entry, raw device I/O, line discipline, file I/O, exact output, canonical display conversion, refcounted strings, and abort-only panic behavior. Stage 19 ownership, moves, destruction, and native class layout is next. Do not recreate a parallel native IR, bypass shared MIR validation, or execute user code as a native-compilation preflight.
