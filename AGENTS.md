...
- Stages 30a through 30f are complete. Closure execution agrees across the debug interpreter, Cranelift fast profile, LLVM release profile, and the PHP compatibility backend for its supported value surface. Keep native closure descriptors static and lean, use the two-word descriptor/environment carrier, allocate no environment for no-capture closures, use stack storage for proven nonescaping native environments, and use one heap allocation for escaping native environments. PHP must consume semantic and validated MIR closure plans and use explicit compiler-generated carriers, environments, and BindingId-backed stable places; PHP automatic capture, `use (...)`, host callable semantics, and PHP references never define Doria behavior. Acquire captures in source order and release remaining owned captures in reverse logical order. Ordinary closures must not use reference counting. `E0641` is unreachable for supported plain closure execution but remains catalogued until Stage 30h; Stage 30g List algorithms are next and Stage 30 is not complete.
