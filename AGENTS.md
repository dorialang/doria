...
- Stage 11 is MIR + interpreter oracle. Stage 11a seeds MIR dumps and the interpreter for a tiny executable subset only. Stage 11b expands MIR/debug interpreter coverage to int expressions, readonly/writable int locals, simple writable int mutation, and checked `+`, `-`, `*` arithmetic. It still does not complete Stage 11, and new language capability work should go through MIR/interpreter rather than NativeSmokeModule.
