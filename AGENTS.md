...
- Stages 11 and 12 are complete. The debug interpreter and Cranelift consume the same MIR; the durable manifest at `crates/doriac/tests/fixtures/native_parity_examples.txt` drives exact stdout/stderr/status differential tests; recursion and general supported control flow share that path; and the former Stage 7-10 native smoke module has been deleted. `doria-rt` owns native entry, exact output, and abort-only panic behavior. Stage 13 is next: the remaining fixed-width integer types and integer operators. Do not recreate a parallel native IR, bypass MIR, or execute user code as a native-compilation preflight.
...
- Do not claim runtime strings, interpolation, formatted output, `Console`, or general runtime I/O are implemented because compile-time-known native `echo` and panic messages work. Stage 12 adds exact byte output and panic status 101 only.
- Treat panic as fatal, non-catchable, and non-unwinding. Explicit `panic("message")`, checked `int` `+`/`-`/`*` overflow, and an invalid `main(): int` process status use the abort-only status-101 path defined by decisions 0040 and 0044.
...
When native backend work changes supported behavior, run both linker-independent MIR-to-Cranelift object tests and the complete durable manifest-driven interpreter/Cranelift differential suite. The current Stage 12 target supports top-level free functions, one parameterless `main`, path-sensitive returns, recursion and mutual recursion, general supported nested control flow, integer locals and checked `+`/`-`/`*`, `int` helper parameters, `int`/`void` helper returns, compile-time-known readonly string-expression echo, and abort-only panic. Normal interpretation has no artificial block or call-depth cap, and native compilation has no interpreter preflight. Helper `int` values retain the full Doria range; only `main(): int` crosses the `0..125` process-status boundary. Do not treat the internal `doria-rt` ABI as stable or claim runtime strings, classes, collections, ownership, LLVM, or broader Stage 13+ support.
