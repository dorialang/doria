...
- Stage 11 is complete. All accepted Stage <=10 executable lowering passes through MIR; the debug interpreter and Cranelift consume the same MIR; the manifest at `crates/doriac/tests/fixtures/stage_11_native_parity_examples.txt` drives exact stdout/status differential tests; and the former Stage 7-10 native smoke module has been deleted. Stage 12 is next: broader control flow plus the minimal doria-rt runtime/panic foundation. Do not recreate a parallel native IR or bypass MIR for new capability work.
...
When native backend work changes supported behavior, run both linker-independent MIR-to-Cranelift object tests and the complete manifest-driven interpreter/Cranelift differential suite. The current Stage 11 target supports top-level free functions, one parameterless `main`, integer locals and checked `+`/`-`/`*`, typed conditions, structured `if` and loops, `int` helper parameters, `int`/`void` helper returns, and compile-time-known readonly string-expression echo. The interpreter preflight remains bounded, helper `int` values retain the full Doria range, and only `main(): int` crosses the `0..125` process-status boundary. Do not treat this as stable ABI, runtime strings, recursion, classes, collections, doria-rt, LLVM, or broader Stage 12+ support.
