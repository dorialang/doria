...
- Stages 11 through 14 are complete. The debug interpreter and Cranelift consume the same typed MIR; the durable manifest at `crates/doriac/tests/fixtures/native_parity_examples.txt` drives exact stdout/stderr/status differential tests and must cover every finite `examples/native/*.doria` fixture. General control flow, recursion, all fixed-width integers, IEEE binary32/binary64 floats, runtime bool values, and explicit numeric conversions share that path. `doria-rt` owns native entry, exact output, and abort-only panic behavior. Stage 15 is next: an LLVM release backend over the same MIR. Do not recreate a parallel native IR, bypass MIR, or execute user code as a native-compilation preflight.
...
- Treat panic as fatal, non-catchable, and non-unwinding. Explicit `panic("message")`, checked-integer failures, invalid `Float::toInt`, and an invalid `main(): int` process status use the abort-only status-101 path defined by decisions 0040, 0041, 0042, 0044, and 0072.
...
When native backend work changes supported behavior, run both linker-independent MIR-to-Cranelift object tests and the complete durable manifest-driven interpreter/Cranelift differential suite. The current Stage 14 target supports top-level free functions, one parameterless int/void `main`, path-sensitive returns, recursion, supported structured control flow, every fixed-width integer type, IEEE binary32/binary64 values, runtime bool values, scalar locals/parameters/returns/calls, bool short-circuiting, and the accepted explicit numeric conversions. It retains compile-time-known readonly string-expression echo and abort-only panic. Normal interpretation has no artificial block or call-depth cap, and native compilation has no interpreter preflight. Only `main(): int` crosses the `0..125` process-status boundary. The durable manifest must include every finite native example and compare exact stdout, stderr, and status, including integer and float-conversion panic fixtures. Do not treat the internal `doria-rt` ABI as stable or claim runtime strings, classes, collections, ownership, LLVM, or broader Stage 15+ support.
