...
- In two-binding `foreach`, treat the first source binding as syntax-neutral until semantics classifies it. `List<T>` and `T[]` provide a readonly zero-based canonical-`int` sequence index; Dictionary families provide their actual readonly key. Ranges, sets, deques, and Dictionary projections remain value-only. Do not infer this matrix again in HIR, MIR, a backend, or tooling.
