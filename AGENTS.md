...
When a native backend target exists, native smoke tests must be part of the relevant definition of done. The current native target is the Stage 4a Cranelift-backed smoke backend for final returns and narrow terminal `if` / `else` returns only; do not treat it as full native code generation or full native control-flow support.
