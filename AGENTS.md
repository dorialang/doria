...
- Treat basic `if` / `else if` / `else`, `while`, and unlabeled `break;` / `continue;` as MVP control flow. `if` is statement control flow and does not return a value; `when` is the planned value-returning conditional/control construct. `break` exits the nearest enclosing loop, and `continue` jumps to the next nearest-loop iteration. Keep `finally`, `do ... while`, `given`, value-returning `when`, `match`, labeled or numeric loop control, and broader control-flow semantics as planned implementation work until their remaining grammar and semantics are specified.
