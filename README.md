...
- Lowers the Stage 11d debug subset to deterministic MIR basic blocks with typed conditions, structured `if` / `else if` / `else`, structured `while`, nested loop control, integer mutation, and fallthrough; the MIR interpreter executes that subset as the growing semantic-oracle path.
...
`compile` defaults to native output and infers an output file name from the input file. `php` is implemented as an explicit compatibility backend. The `debug` target emits a Stage 11d MIR interpreter artifact for inspection, while `wasm` remains planned.
