...
- Lowers the Stage 11c debug subset to deterministic MIR basic blocks with typed conditions, structured `if` / `else if` / `else`, branch-local integer work, and fallthrough; the MIR interpreter executes that subset as the growing semantic-oracle path.
...
`compile` defaults to native output and infers an output file name from the input file. `php` is implemented as an explicit compatibility backend. The `debug` target emits a Stage 11c MIR interpreter artifact for inspection, while `wasm` remains planned.
