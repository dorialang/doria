...
The native backend currently supports a narrow Stage 9 smoke shape: exactly one top-level `function main(): int` or `function main(): void`. `main(): int` returns an explicit process status in the portable `0..125` exit-code range. `main(): void` may fall through or use `return;` and exits successfully with status `0`. The supported source subset includes readonly and writable integer locals, `=`, `+=`, and `-=` assignments to writable integer locals, standalone integer `++` / `--`, `+`/`-`/`*` arithmetic, structured returning `if` blocks, fallthrough `if` statements that merge visible integer locals, bounded/proven structured `while` loops, bounded/proven traditional `for` loops, and bounded/proven integer range `foreach`. Supported native loop bodies may contain integer local declarations, readonly string locals initialized from supported compile-time-known string expressions, writable integer assignments, fallthrough `if` statements, `break`, `continue`, and supported string `echo` expressions. Native validation proves accepted loops terminate within the current smoke verification cap before lowering them through a private native smoke module to real Cranelift control flow. Conditions support bool literals, grouped conditions, integer comparisons over supported integer expressions, `!` / `not`, `&&` / `and`, `||` / `or`, and `xor`. String-literal, readonly string-local, and compile-time-known string-concat `echo` write exact bytes to stdout with no implicit newline; heap strings, writable string locals, string assignment, interpolation, runtime concatenation, formatting, and broader runtime I/O remain future work.
...
- The native backend currently accepts only the Stage 9 smoke subset, `function main(): int` or `function main(): void`, supported readonly and writable integer locals, writable integer assignments, standalone integer `++` / `--`, `+`/`-`/`*` arithmetic, structured returning `if` blocks, fallthrough `if` statements with visible-local merges, bounded/proven structured `while` loops, bounded/proven traditional `for` loops, bounded/proven integer range `foreach`, supported loop-body integer locals, readonly string locals initialized from supported compile-time-known string expressions, writable integer assignments, fallthrough `if` statements, `break`, `continue`, accepted boolean conditions, exact string-literal `echo` stdout, supported readonly string-local `echo`, and compile-time-known string-concat `echo`. It rejects broader valid Doria with unsupported-feature diagnostics. The `0..125` range is a process-exit boundary for explicit `main(): int` status values, not the range of Doria `int`.
