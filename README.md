...
- The native backend currently accepts only the Stage 9 smoke subset, `function main(): int` or `function main(): void`, supported readonly and writable integer locals, writable integer assignments, standalone integer `++` / `--`, `+`/`-`/`*` arithmetic, structured returning `if` blocks, fallthrough `if` statements with visible-local merges, bounded/proven structured `while` loops, bounded/proven traditional `for` loops, bounded/proven integer range `foreach`, supported loop-body integer locals, readonly string locals initialized from string literals, assignments, fallthrough `if` statements, `break`, `continue`, accepted boolean conditions, exact string-literal `echo` stdout, and supported readonly string-local `echo`. It rejects broader valid Doria with unsupported-feature diagnostics. The `0..125` range is a process-exit boundary for explicit `main(): int` status values, not the range of Doria `int`.
