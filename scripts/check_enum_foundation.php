...
$require($semanticsPath, $semantics, [
    ...
    'Payload Enum Execution Lands In Stage 27 Slice 2',
...
foreach ([$interpreterPath => $interpreter, $craneliftPath => $cranelift, $llvmPath => $llvm] as $path => $contents) {
    $require($path, $contents, ['EnumExpression', 'EnumBacking']);
}
$require($phpPath, $php, ['fn emit_enum(', 'output.push_str("enum ")', 'output.push_str(": ")']);
$require($testsPath, $tests, [
    ...
    'payload_and_match_syntax_stop_once_at_their_owned_semantic_boundaries',
]);
