...
- Treat `docs/decisions/0033-stage-8a-native-readonly-string-locals-and-echo.md` as the accepted Stage 8a direction for literal-backed readonly string locals and supported string `echo` expressions.
- Keep broader native expression, assignment, and control-flow support beyond the Stage 8a smoke subset as separate future implementation slices.
- Keep broader native strings, writable string locals, interpolation, concatenation, runtime I/O, stdout formatting, and nonzero runtime error statuses as future runtime/error-model work.
...
- Expand native source support beyond Stage 8a only after the next accepted native slice specifies the language semantics and expected behavior.
