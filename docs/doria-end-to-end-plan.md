...
- **`float32` variants and SIMD**: record 0065 (SIMD direction) treats `Doria\Std\Math` as its first consumer — `Vector3`/`Vector4` over `float32` map directly onto SIMD lanes, so layout decisions in 0074 and 0065 are made together.
...
### Record 0085 detail — the namespace model

Elaborates 0028 and feeds 0058's implementation notes. Found by the designer's BDD/playground UAT (a real `Assegai\Core\Http\JsonResponse` fixture), which is also why §0's two-clocks rule now exists.
