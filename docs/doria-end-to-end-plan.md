...
- Decision 0120: explicit closure capture lists — arrows and anonymous block functions use the same explicit `with` list for every enclosing local dependency; readonly, writable, and taking modes are written; Copy and Move values receive no implicit exception; no-capture closures omit `with`; Stage 30 owns capture checking, callable-effect integration, and accepted closure algorithms while `$this` remains a bounded Stage 30 question.
...
- **Stage 30 — Closures.** Decisions 0120, D10, D20, and 0119: arrow and anonymous-function grammar; explicitly typed parameters; arrow return inference; function types; explicit `with` capture lists with readonly, writable, and taking modes; missing-capture diagnostics and fixes; capture validation, ownership, environment representation, calls, checked-effect integration, and borrow-bound escape checking; Decision 0100's accepted `List<T>` `map`/`filter`/`reduce`; interpreter, Cranelift, LLVM, PHP compatibility, language-server, and website/playground activation. PHP lowering follows Doria's written captures, not PHP defaults. Async/cross-task capture remains in the later structured-concurrency stage; general erased interface interaction remains Stage 35; `$this`, closure effect inference/annotation, and implementation slicing remain bounded Stage 30 design work. **Stage 30 remains blocked until Stage 29 completes and is not implemented.**
