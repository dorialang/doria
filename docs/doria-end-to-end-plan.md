...
- **`\` is also the string escape.** Qualified names in source are code tokens and need no quoting. When a namespace or Windows path is represented as string data, each backslash must currently be escaped as `\\` in either quote form; single-quoted strings are not a raw-string escape hatch. Any future change to single-quoted escape behavior requires its own explicit language decision and tests.
...
- **Stage 31 — Namespaces/use/include/declare.** Decision 0028 implemented as elaborated by the namespace-model decision (resolution model, prelude, group `use`); multi-file compilation; first declare keys. Qualified names must resolve in **every** type position, not only declarations: `extends`, `implements`, parameter/return/property/`let` annotations, `new`, static calls (`Vendor\Lib\Counter::initialValue()`), constant access, and `is` operands. The constant-access and `is` grammar lands in Stages 20 and 22 respectively; Stage 31 adds name resolution rather than retroactively owning those syntax forms. AC: multi-file example project builds; duplicate-symbol diagnostics; Doria stdlib fragments that were compiler-provided in Stage 26 now compile through `include`; a qualified-name fixture exercising every position whose grammar has landed; **absolute-on-`\` fixtures** — a name containing `\` resolves identically regardless of enclosing namespace, and the same-prefix case (`namespace Assegai\Core\Http; class JsonResponse extends Assegai\Core\Http\Response`) resolves to the real class, never PHP's doubled `Assegai\Core\Http\Assegai\Core\Http\Response`; group-`use` and alias fixtures; prelude-shadowing fixtures (a userland `List` wins; a userland `Displayable` is rejected per 0079); intrinsic-redeclaration rejection (`function read_line()` errors as `panic` already does); **LSP no-false-diagnostics test** per §0 — the accepted-syntax fixture yields zero parser errors and only stable semantic unsupported-feature diagnostics before inheritance and interface conformance land.
