...
- **Stage 30 Closure Authority — Accepted; Stage 30 — In Progress, Not Complete.** Decision 0121 elaborates Decision 0120 and applies Decision 0119. Function values are Move-only structural values with independent value ownership, readonly/writable/once invocation, and per-argument ownership. Function types preserve checked effects; arbitrary callable expressions invoke positionally after one callee evaluation; named and bound callable references remain deferred. `$this` is explicit, capture discovery uses stable binding identities, acquisition happens at creation, borrow escape is checked, and runtime representation is a lean two-word descriptor/environment carrier whose private fields may reorder without changing logical source order. E0641 remains active and retires only by implemented route. Controlled timing remains non-blocking with `Measurement Status: Pending Available Runner`.

  **Accepted dependency order:** Authority Acceptance — Complete; Stage 30a Callable Grammar Completion — Complete; **Stage 30b Semantic Function Types And Captures — Next**; Stage 30c Ownership, Lifetime, And Escape; Stage 30d Closure HIR/MIR And Interpreter Oracle; Stage 30e Native Execution; Stage 30f PHP Compatibility; Stage 30g List Algorithms; Stage 30h Cross-Repository Closure. Stage 30a changes syntax only; no semantic or execution slice is complete.
