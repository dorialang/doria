...
These types land at Stage 25a (thread-safe variants at Phase H); they depend on nullable types and narrowing (Stage 22) and generic classes (Stage 25).
...
- `mixed` remains the dynamic escape hatch for PHP-interop shapes; Stage 22 narrows it through explicit `is` checks (`$x is string`), while Stage 28 adds `match`. `mixed` is deliberately unknown-flavored and boxed — Stage 22 implements its static rules, Stage 23 its runtime representation, and its complete language rules live in §4.8.
...
- `when` is the value-returning form of `if` — the same `given` / `else when` / `else` / `finally` structure (`when`/`else when` in place of `if`/`else if`), always yielding a value (declared result type, mandatory `else`, every branch `return`s the value). Its grammar is settled in decision 0097 (records 0009/0020 are its ancestors). It lands at Stage 28a (control-flow completion), right after `match`.
...
- Decision 0097: the `when` value-returning control construct — `when` is the value-returning form of `if` (same `given`/`else when`/`else`/`finally` structure), differing only in that it always yields a value: declared result type, mandatory total `else`, block-scoped `return`-to-yield per branch, expression position, and a false `given` predicate routing to `else`.
...
- Property hooks. The demanding design case is **ORM-shaped lazy-loaded relations**. PHP/TypeScript ORMs in the AssegaiPHP/TypeORM lineage use proxies to intercept property access; Doria has no reflection and no proxies, so hooks are the only mechanism a property-shaped lazy relation could use. The record must decide whether a hook may perform I/O, `throws`, or blocking work, and how that interacts with a property-shaped API the §6 charter says should look like data. Design against this case rather than only simple validation and computed properties.
- Closures (D10).
...
- `Shared<T>` / `Weak<T>` / `SharedMut<T>` (authored at Stage 25a).
...
- **Stage 25a — Opt-in shared ownership.** The §3.3 pressure valve builds on decision 0005's `shared new` surface and on Stage 22 nullable/narrowing plus Stage 25 generic classes: `Shared<T>` is a refcounted owner with cheap handle `->clone()` and last-handle destruction; `Weak<T>` is non-owning and `->upgrade(): ?Shared<T>`; `SharedMut<T>` provides runtime-checked `writable` access that panics on overlapping access. Thread-safe variants remain in Phase H under the `Sendable`/`Shareable` record. The §12 shared-ownership types/API decision is authored when this stage is implemented, against the real generics machinery. AC: `SharedMut<T>` emits dynamic access checks and panics with a clear message on overlapping writable access; ordinary borrow checking emits zero runtime checks even alongside `SharedMut<T>`; `Shared<T>` handle-clone/last-drop-destruction and `Weak<T>` upgrade-to-`?Shared<T>` fixtures; a documented strong-cycle-leak/`Weak`-breaks-cycle fixture; interpreter/Cranelift/LLVM parity.
...
- **Stage 44 — SIMD direction (the SIMD/engine-intrinsics decision)** + `Doria\Std\Net`/`Http` maturation for the PHP sidecar pattern.
