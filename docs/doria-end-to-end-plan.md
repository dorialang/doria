...
- Decision 0128: Baton workspaces, development, tests, processors, and project inventory — Stage 33 Slice 3 fixes canonical source transport/locator spellings, workspace lock schema 2, development graphs, compiler-metadata-driven tests, one-pass processors, generated-source ownership, non-executing graph inspection, incremental project inventory, and asynchronous language-server consumption. Implementation is in progress across the compiler, Baton bootstrap, and language server.
...
- **Interactive Line-Input Amendment — Implemented.** Amends decision 0074's
  ...
  **Current planning checkpoint:**
  ...
  - **Stage 33:** Slice 1 — Complete under Decision 0126; Slice 2 — Complete under Decision 0127; Slice 3 — In Progress under Decision 0128; Stage 33 — In Progress, Not Complete.
  - **Stage 33 Slice 3 — In Progress:** Complete Baton and language-server integration; compiler metadata schema 2 is implemented.
...
- **Stage 29 — Checked errors end-to-end — Complete.** Decision 0119 settles and implements the complete model.
  ...
  - **Continuation:** The pre-Stage-30 closure grammar slice, Decision 0121 authority beat, Stages 30a through 30h, and the Decision 0122 corrective beat are complete; Stages 30 through 32 and Stage 33 Slices 1 and 2 are complete, and Stage 33 Slice 3 is in progress under Decision 0128.
...
- **Stage 30 Closure Authority — Accepted And Implemented; Stage 30 — Complete.** Decision 0121 elaborates Decision 0120 and applies Decision 0119. Function values are Move-only structural values with independent value ownership, readonly/writable/once invocation, and per-argument ownership. Function types preserve checked effects; arbitrary callable expressions invoke positionally after one callee evaluation; named and bound callable references remain deferred. The subsection order below is the accepted dependency order.
  ...
  - **Continuation:** Stages 31 and 32 and Stage 33 Slices 1 and 2 are complete; Stage 33 Slice 3 is in progress under Decision 0128. The debug interpreter remains the semantic oracle; Cranelift, LLVM, and PHP compatibility consume the same validated closure authority for their supported surfaces.
...
- **Stage 33 — Baton package and dependency workflow in the disposable PHP UX bootstrap — In Progress, Not Complete.** Decisions 0118, 0124, 0126, 0127, and 0128. Stage 33 deliberately completes and exercises the durable Baton product contract in `dorialang/baton-php`; it does not promote PHP to Baton's permanent implementation language and does not satisfy the Doria-native transition.
