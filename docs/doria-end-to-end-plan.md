...
- **The PHP-familiar free-function layer** — regularized `snake_case` wrappers over the member/companion surface (`get_time`, `str_starts_with`, `str_case_compare`, ...), never PHP's fused spellings.
...
- Decision 0103: String companion surface — pure string transforms live on the `String` companion (`String::trim`/`String::lower`/`String::upper` seed set, fuller surface TBD with the strings work), matching the `Int`/`Float` companions. Draws the one-home-per-operation boundary: `$s->` for intrinsic properties (`length`/`isEmpty`/`bytes`/`chars`), `String::` for transforms, and the `str_*` free-function family for the PHP-familiar predicate/search layer — with no `str_trim`/`str_lower` duplicating the companion transforms. Ratifies the `String::trim`/`String::lower` the website guides already teach.
...
- **Diagnostic Experience Foundation — Implemented.** Decision 0108 replaces the
  ...
  structure. This checkpoint does not complete Stage 25a Slice 4. The String API
  Decision Amendment remains the next language work.
