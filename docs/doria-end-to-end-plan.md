...
- **Interactive Line-Input Amendment — Implemented.** Amends decision 0074's
  ...
  **Current planning checkpoint:**
  ...
  - **Native Testing Foundation:** Slice 1 — Complete under Decision 0129; Slice 2 — Compiler/Runtime Implemented, Baton/Tooling Pending; Foundation — In Progress, Not Complete.
...
- **Stage 29 — Checked errors end-to-end — Complete.** Decision 0119 settles and implements the complete model.
  ...
  - **Continuation:** The pre-Stage-30 closure grammar slice, Decision 0121 authority beat, Stages 30a through 30h, and the Decision 0122 corrective beat are complete; Stages 30 through 33 and Phase F are complete, Native Testing Foundation Slice 1 is complete, and Slice 2 precedes Stage 34.
...
- **Stage 30 Closure Authority — Accepted And Implemented; Stage 30 — Complete.** Decision 0121 elaborates Decision 0120 and applies Decision 0119. Function values are Move-only structural values with independent value ownership, readonly/writable/once invocation, and per-argument ownership. Function types preserve checked effects; arbitrary callable expressions invoke positionally after one callee evaluation; named and bound callable references remain deferred. The subsection order below is the accepted dependency order.
  ...
  - **Continuation:** Stages 31 through 33 and Phase F are complete; Native Testing Foundation Slice 1 is complete, and Slice 2's compiler/runtime portion is implemented while Baton/tooling coordination remains pending. The debug interpreter remains the semantic oracle; Cranelift, LLVM, and PHP compatibility consume the same validated closure authority for their supported surfaces.
...
- **Stage 33 — Baton package and dependency workflow in the disposable PHP UX bootstrap — Complete.** Decisions 0118, 0124, 0126, 0127, and 0128. Stage 33 completes and exercises the durable Baton product contract in `dorialang/baton-php`; it does not promote PHP to Baton's permanent implementation language and does not satisfy the Doria-native transition.
  ...
  - **Phase F — Complete.**
  - **Continuation:** Native Testing Foundation Slice 1 is complete; Slice 2's compiler/runtime portion is implemented while Baton/tooling coordination remains pending; and Stage 34 single class inheritance waits for the foundation to complete. Decision 0124's Pre-Stage-45 Doria-native Baton transition remains scheduled and mandatory before the first unsuffixed `2026.03.1` release.

- **Pre-Stage-34 Native Testing Foundation — In Progress, Not Complete.** Decision 0129. Compiler-owned test declarations and metadata extend the completed Stage 33 runner without reopening Phase F.
  ...
  - **Slice 2 — Compiler/Runtime Implemented, Baton/Tooling Pending:** Implements `expect`, `fail`, ephemeral expectations, the `not` property, scalar/string/null/bool/ordered assertions, `AssertionError`, the separate `TestAssertion` effect, strict DORIAO4 outcomes, cleanup, and interpreter/Cranelift/LLVM/PHP parity without a runtime expectation object or mandatory success allocation. Baton regression integration and official language-server pin/fact coordination remain before completion.
  - **Slice 3 — Later:** Implements collection and dictionary expectations, typed Error expectations, bounded differences, final Baton reporting/outcome classification, complete tooling navigation, documentation examples, and installed-toolchain closure.
  - **Status:** Stage 33 — Complete; Phase F — Complete; Native Testing Foundation Slice 1 — Complete; Native Testing Foundation Slice 2 — Compiler/Runtime Implemented, Baton/Tooling Pending; Native Testing Foundation — In Progress, Not Complete; Stage 34 Single Class Inheritance — Blocked Until The Foundation Completes.

### Phase G — OOP completion and hosted I/O foundation (Stages 34–36a)
- **Stage 34 — Inheritance — Blocked Until The Native Testing Foundation Completes.** Qualified `extends` names (parsed since the §0 two-clocks rule, resolved since Stage 31) become fully semantic here. D17: `open`/`override`, vtables, parent construction rules, devirtualization in LLVM profile. AC: `Post extends Model` native; missing-override diagnostics.
