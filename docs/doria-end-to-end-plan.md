...
- **Interactive Line-Input Amendment — Implemented.** Amends decision 0074's
  ...
  is complete, and decision 0110 accepts the stream architecture and performance contract. Stage 26 and Stage 26a are complete; Stage 26b is in progress with decision 0112 accepted, Slices 1 and 2 complete, Slice 3 blocked on controlled Linux acceptance evidence, and Stage 27 blocked until Stage 26b completes.

  **Current planning checkpoint:** Stage 25a — Complete; PHP Stream And I/O Completeness Audit — Implemented; Andrew’s Stream API Completeness Review — Complete; Stream Architecture And Performance Decision — Accepted (decision 0110); Stage 26 — Complete; Stage 26a — Complete; Stage 26b — In Progress, Slices 1 And 2 Complete, Slice 3 Blocked On Controlled Linux Evidence (decision 0112); Stage 26b Controlled Linux Acceptance Evidence — Next; Stage 27 — Blocked Until Stage 26b Completes; Stage 35a — Scheduled; Stage 36a — Scheduled; Stage 36a Public Spellings — Deferred; Stage 36a — Not Implemented.
...
- **Stage 26b — Performance Baseline Foundation — In Progress; Slices 1 And 2 Complete, Slice 3 Blocked On Controlled Linux Evidence.** Decision 0112 accepts one repository-owned benchmark system that later stages extend; this is measurement infrastructure and accepted baselines, not an unlimited optimization campaign. Slice 1 supplies the strict manifest/report and compiler-report foundation. Slice 2 supplies deterministic scaling, the compiler/generated/runtime/diagnostic matrix, process-resource and profiler adapters, candidate evidence, and an exact structural baseline without timing thresholds. Slice 3 supplies comparative peers, enforced fairness records, controlled-runner and promotion workflows, deterministic compiler/runtime artifact identity, shared workload profiles, and the exact native acceptance policy. It keeps compiler performance, generated-program performance, runtime-subsystem performance, cold startup, memory, and artifact size as separate dimensions. The interpreter remains a semantic oracle rather than a native competitor. The native runtime status compares each Doria backend with the fastest valid C, C++, or Rust peer and passes only at `ratio <= 1.30`; PHP remains adoption evidence. Correctness and adequacy precede timing, Inconclusive is never Pass, and Doria self-regression thresholds do not weaken cross-language status. No eligible controlled Linux sessions exist, so Slice 3 cannot promote timing baselines or close. A future `baton bench` orchestrates the same engine. Stage 27 remains blocked until all slices complete. AC: reproducible manifest-driven runner and report schema; exact-output/hash validation; complete acceptance matrix on an eligible controlled Linux runner; recorded Doria regression baselines; deterministic CI structural checks.
  Slice 3 now binds the exact native acceptance rule: Doria must be no more than 30% slower than the fastest valid C, C++, or Rust peer (`ratio <= 1.30`); PHP remains adoption evidence and Inconclusive is never Pass. Deterministic runtime construction and compiler/runtime SHA-256 identity close the earlier reproducibility defect. No eligible controlled Linux sessions exist, so no timing baseline or acceptance matrix is promoted and the stage remains blocked.
  **Slice status:** Stage 26b — In Progress; Stage 26b Slice 1 — Complete; Stage 26b Slice 2 — Complete; Stage 26b Slice 3 — Blocked On Controlled Linux Evidence; Stage 26b Controlled Linux Acceptance Evidence — Next.
