...
- **Stage 29 — Checked errors end-to-end — Complete.** Decision 0119 settles and implements the complete model.
  ...
  - **Continuation:** The pre-Stage-30 closure grammar slice, Decision 0121 authority beat, Stages 30a through 30h, and the Decision 0122 corrective beat are complete; Stages 30 through 33, Phase F, and Native Testing Foundation Slices 1 through 3 are complete. Stage 34 is next.
...
- **Stage 30 Closure Authority — Accepted And Implemented; Stage 30 — Complete.** Decision 0121 elaborates Decision 0120 and applies Decision 0119. Function values are Move-only structural values with independent value ownership, readonly/writable/once invocation, and per-argument ownership. Function types preserve checked effects; arbitrary callable expressions invoke positionally after one callee evaluation; named and bound callable references remain deferred. The subsection order below is the accepted dependency order.
  ...
  - **Continuation:** Stages 31 through 33, Phase F, and Native Testing Foundation Slices 1 through 3 are complete; Stage 34 is next. The debug interpreter remains the semantic oracle; Cranelift, LLVM, and PHP compatibility consume the same validated closure authority for their supported surfaces.
...
- **Stage 33 — Baton package and dependency workflow in the disposable PHP UX bootstrap — Complete.** Decisions 0118, 0124, 0126, 0127, and 0128. Stage 33 completes and exercises the durable Baton product contract in `dorialang/baton-php`; it does not promote PHP to Baton's permanent implementation language and does not satisfy the Doria-native transition.
  ...
  - **Phase F — Complete.**
  - **Continuation:** Native Testing Foundation Slices 1 through 3 are complete and Stage 34 single class inheritance is next. Decision 0124's Pre-Stage-45 Doria-native Baton transition remains scheduled and mandatory before the first unsuffixed `2026.03.1` release.
...
- **Pre-Stage-34 Native Testing Foundation — Complete.** Decision 0129. Compiler-owned test declarations, expectations, outcomes, Baton reporting, and tooling extend the completed Stage 33 runner without reopening Phase F.
  ...
  - **Status:** Stage 33 — Complete; Phase F — Complete; Native Testing Foundation Slice 1 — Complete; Native Testing Foundation Slice 2 — Complete; Native Testing Foundation Slice 3 — Complete; Native Testing Foundation — Complete; Stage 34 Single Class Inheritance — Next; Pre-Stage-45 Doria-Native Baton Transition — Scheduled.

### Phase G — OOP completion and hosted I/O foundation (Stages 34–36a)
- **Stage 34 — Single Class Inheritance — Next.** Qualified `extends` names (parsed since the §0 two-clocks rule, resolved since Stage 31) become fully semantic here. D17: `open`/`override`, vtables, parent construction rules, devirtualization in LLVM profile. AC: `Post extends Model` native; missing-override diagnostics.
