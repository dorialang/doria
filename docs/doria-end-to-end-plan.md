...
- `when` is the value-returning form of `if` — the same `given` / `else if` / `else` / `finally` structure, always yielding a value (declared result type, mandatory `else`, every branch `return`s the value). Its grammar is settled in decision 0097 (records 0009/0020 are its ancestors). It lands at Stage 28a (control-flow completion), right after `match`.
...
- Decision 0097: the `when` value-returning control construct — `when` is the value-returning form of `if` (same `given`/`else if`/`else`/`finally` structure), differing only in that it always yields a value: declared result type, mandatory total `else`, block-scoped `return`-to-yield per branch, expression position, and a false `given` predicate routing to `else`.
