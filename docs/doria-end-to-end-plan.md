...
- Flow-sensitive narrowing: `!= null` / `== null` comparisons and exact `is` tests establish non-null, null, or exact-type facts inside the guarded region. Stage 22 implements these facts on the shared forward-dataflow framework; Stage 28 adds `match` as another fact source.
...
- `mixed` remains the dynamic escape hatch for PHP-interop shapes; Stage 22 narrows it through explicit `is` checks (`$x is string`), while Stage 28 adds `match`. `mixed` is deliberately unknown-flavored and boxed — Stage 22 implements its static rules, Stage 23 Slice 3 implements its runtime representation, and its complete language rules live in §4.8.
...
- Decision 0114 owns the complete enum semantics. Stage 27 Slice 1 executes
  unit and `int`/`string`-backed enums with nominal identity, inline tags,
  equality, readonly backed `value`, nullability, `mixed`, constants, and
  supported collection placement. Payload schemas parse and stop before MIR;
  Slice 2 owns payload execution and aggregate layout.
...
- **Interactive Line-Input Amendment — Implemented.** Amends decision 0074's
  ...
  is complete, and decision 0110 accepts the stream architecture and performance contract. Stage 26, Stage 26a, and Stage 26b are complete; the performance `Measurement Status: Pending Available Runner` is non-blocking, all four Decision 0113 slices and Decision 0113 overall are complete, and Stage 27 is in progress with Slice 1 complete and Slice 2 next.

  **Current planning checkpoint:** Stage 25a — Complete; PHP Stream And I/O Completeness Audit — Implemented; Andrew’s Stream API Completeness Review — Complete; Stream Architecture And Performance Decision — Accepted (decision 0110); Stage 26 — Complete; Stage 26a — Complete; Stage 26b — Complete, All Three Slices Complete (decision 0112); Measurement Status: Pending Available Runner; Decision 0113 Slice 1 — Complete; Decision 0113 Slice 2 — Complete; Decision 0113 Slice 3 — Complete; Decision 0113 Slice 4 — Complete; Decision 0113 — Complete; Stage 27 — In Progress; Stage 27 Slice 1 — Complete; Stage 27 Slice 2 — Next; Stage 28 — Blocked Until Stage 27 Completes; Stage 35a — Scheduled; Stage 36a — Scheduled; Stage 36a Public Spellings — Deferred; Stage 36a — Not Implemented.
...
- **Stage 27 — Enums + payload cases — In Progress; No Performance-Evidence Dependency.** Decision 0114. **Slice 1 — Complete:** authority, enum/case grammar, unit and `int`/`string`-backed execution, nominal identity, inline tags, equality, readonly `value`, nullable/`mixed`, constants/statics/defaults, supported collection placement, PHP native enums, and grammar-only payload/`match` preparation. **Slice 2 — Next:** payload execution, finite inline payload layout, conditional Copy/move/drop glue, aggregate ABI, all storage positions, payload equality/`mixed`, and PHP payload lowering. AC: Slice 1 exact interpreter/Cranelift/LLVM parity; `Shape` example native at Stage 27 closure.
- **Stage 28 — match — Blocked Until Stage 27 Completes.** D7, exhaustiveness, payload destructuring, narrowing integration; guards fast-follow within the stage. Grammar parses now and emits one Stage 28 semantic boundary diagnostic. AC: exhaustiveness diagnostics snapshots; `match (true)` chains.
