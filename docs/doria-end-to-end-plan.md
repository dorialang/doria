...
- Stream, Readiness, Standard I/O, And Blocking-Mode Model: capability interfaces; ownership/close; byte read/write results; EOF/would-block/partial progress; blocking mode; readiness; timeouts/deadlines; first-class standard streams; buffering/text adapters; files; process pipes; platform differences; and async, terminal, and network boundaries (Stage 36a). This subject is authored only after Andrew reviews the PHP Stream And I/O Completeness Audit.
...
- **Interactive Line-Input Amendment — Implemented.** Amends decision 0074's
  ...
  is the next checkpoint, and Stage 26 is blocked only until that review completes.

  **Current planning checkpoint:** Stage 25a — Complete; PHP Stream And I/O
  Completeness Audit — Implemented; Andrew’s Stream API Completeness Review —
  Next; Stage 26 — Blocked Pending Review; Stage 36a — Scheduled; Stage 36a Public
  Surface — Pending Review.
- **Stage 26 — Remaining collection family (blocked pending the Stream API Completeness Review).** Stage 23 ships Decision 0100's default `List`/`Dictionary`/`Set`/`T[]` surface. Stage 26 adds the authored non-closure surface with `SortedDictionary`, `SortedSet`, `PriorityQueue`, and `Deque`; `map`/`filter`/`reduce` remain Stage 30 because they require closures. Before Stage 31 include/multi-file support, required stdlib fragments are compiler-bundled or prelude-style rather than source-included. This temporary review gate does not add streams to Stage 26. AC: the remaining non-closure collection family compiles and runs from the compiler-provided stdlib surface.
...
- **Stage 36a — Stream, readiness, and standard I/O foundation.** Scheduled, not implemented; the public surface is pending Andrew's audit review and the unauthored **Stream, Readiness, Standard I/O, And Blocking-Mode Model** decision. The reviewed subset covers capability-based stream contracts; byte-oriented read/write semantics; explicit EOF, would-block, and partial-write outcomes; first-class standard streams over the existing device substrate; file handles and typed open modes; capability-owned blocking-mode control/discovery; portable readiness waiting; timeout/deadline integration; buffered byte and UTF-8 text/line adapters; streaming copy; process pipes; cross-platform runtime backends; and interpreter/Cranelift/LLVM parity. Prerequisites: Stage 29 checked errors, Stage 31 namespaces/multi-file support, Stage 35 interfaces/capability contracts, existing ownership/RAII, the existing standard-device runtime substrate, and decision 0109's unified diagnostics/runtime outcomes. Property hooks are not intrinsically required, but Stage 36a remains after Stage 36 to preserve the accepted linear sequence. Non-goals: async state-machine lowering, a multithreaded executor, TCP/UDP product APIs, HTTP, TLS implementation, terminal raw mode/key/resize/cursor/screen/color/styling, PHP-compatible dynamic wrapper or string-filter registries, and a global stream context. Preliminary AC, finalized only after public API review: a file copies through generic readable/writable contracts without a whole-file allocation; a child process's stdout and stderr drain without deadlock through readiness-aware pipes; a non-blocking pipe read distinguishes data, would-block, and EOF; a partial write resumes after writable readiness without data loss; first-class stdin/stdout/stderr coexist with unchanged intrinsics on one substrate; Linux/macOS/Windows share one public surface with hidden platform backends; interpreter/Cranelift/LLVM observable behavior agrees. No speculative source example is accepted syntax here.

### Phase H — Concurrency (Stages 37–39)
- **Stage 37 — Concurrency design the async decision.** Paper stage: full async model (executor in doria-rt, task groups, cancellation, `Shareable` rules). **Stage 37 must consume Stage 36a readiness**, stream ownership, backpressure semantics, timeout/deadline model, and process pipes. It must not invent a second stream model, readiness model, event-loop abstraction, or I/O-specific cancellation model. The design tests must include data-pipeline and long-running compute scenarios — bounded parallelism, backpressure, prefetch, worker pools, failure propagation, deterministic cleanup — alongside the PHP thread-affinity invariant (§10.3). Designer sign-off required — this is the one deliberate design gate in the plan.
- **Stage 38 — async/await codegen.** State-machine lowering in MIR; single-threaded executor first; `async main` entry bootstrap per §5 (executor started only when `main` is async — sync programs pay zero async cost). AC: an async file-read example expressed through Stage 36a's stream contracts rather than a one-off file intrinsic; interpreter parity; async-`main` escaping-error example exits 70 with destructors run.
- **Stage 39 — Structured task groups + Shareable checking.** Multithreaded executor; spawn-boundary checks via auto-derived `Sendable`/`Shareable` — with ownership in place this is Rust Send/Sync-grade freedom from data races. Task-group cancellation/failure releases or closes pending stream operations deterministically under Stage 36a's ownership model. AC: parallel map example; data-race fixture rejected at compile time; cancelled stream-operation cleanup fixture.
...
- **Stage 43 — Engine profile.** declare-based overflow relaxation for audited modules; arena allocator hooks in doria-rt; benchmark suite (criterion-style) vs C/Rust baselines for drop-glue, `SharedReference<T>`, and collection hot paths.
- **Stage 44 — SIMD direction (the SIMD/engine-intrinsics decision)** + `Doria\Std\Net`/`Http` maturation for the PHP sidecar pattern. **Stage 44 builds on Stage 36a duplex streams, readiness, timeouts, partial writes, and async integration**; it does not create unrelated socket read/write contracts.
