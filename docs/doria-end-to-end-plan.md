...
- **Interactive Line-Input Amendment — Implemented.** Amends decision 0074's
  ...
  is complete, and decision 0110 accepts the stream architecture and performance contract. Stage 26 and Stage 26a are complete; Stage 26b is in progress with decision 0112 accepted, Slices 1 and 2 complete, Slice 3 in progress with Part 1 delivered and the timing threshold review next, and Stage 27 blocked until Stage 26b completes.

  **Current planning checkpoint:** Stage 25a — Complete; PHP Stream And I/O Completeness Audit — Implemented; Andrew’s Stream API Completeness Review — Complete; Stream Architecture And Performance Decision — Accepted (decision 0110); Stage 26 — Complete; Stage 26a — Complete; Stage 26b — In Progress, Slices 1 And 2 Complete, Slice 3 In Progress (decision 0112); Stage 26b Timing Threshold Review — Next; Stage 27 — Blocked Until Stage 26b Completes; Stage 35a — Scheduled; Stage 36a — Scheduled; Stage 36a Public Spellings — Deferred; Stage 36a — Not Implemented.
...
- **Stage 26b — Performance Baseline Foundation — In Progress; Slices 1 And 2 Complete, Slice 3 In Progress.** Decision 0112 accepts one repository-owned benchmark system that later stages extend; this is measurement infrastructure and accepted baselines, not an unlimited optimization campaign. Slice 1 supplies the strict manifest and report schema, committed exact-output authority, round-robin raw sampling, explicit compiler/driver provenance, opt-in compiler phase reporting, and matched `call_overhead`, `checked_arith`, and `element_access` diagnostic pairs. Slice 2 supplies deterministic compiler scaling, the initial compiler/generated-program/runtime/diagnostic matrix, process-resource and separate optional Callgrind/DHAT adapters, candidate evidence, and an exact structural baseline without timing thresholds. Slice 3 adds peer implementations for the new runtime cases, proposes controlled timing thresholds for a separate review, and closes the stage-completion workflow. Slice 3 Part 1 is delivered: C, Rust, and PHP peers for the seventeen generated-program and runtime-subsystem cases, peer fairness and semantic-equivalence records enforced when the manifest loads, two controlled candidate sessions, controlled-runner and baseline-promotion workflows, and a timing threshold proposal that accepts nothing. It has separate compiler-performance, generated-program-performance, and runtime-subsystem tracks. Compiler measurements include parse, semantic, MIR, Cranelift, LLVM, link, total compile time, peak RSS, source/AST/MIR structure, runtime/output size, and specialization count. Program measurements distinguish cold startup from hot throughput and record latency, wall/user/system time, peak RSS, allocations where available, binary/stripped/compressed size where useful, and correctness output/hash. Runtime cases begin with implemented strings, objects, methods, ownership, collections, generics, and shared ownership; later streams, async, and FFI join at their owning stages. Cranelift targets responsive development compilation/linking with acceptable runtime performance; LLVM targets release runtime, memory, optimization, and artifact size; the interpreter remains the semantic oracle, not a native performance competitor. Initial Doria cases use only implemented language features; C, Rust, and PHP peers for the new runtime cases land in Slice 3. The repository-owned runner records versions, flags, machine, inputs, runner identity, and raw evidence; correctness passes before timing; unfavorable results remain visible; shared CI uses deterministic structural checks while controlled runners own timing thresholds. A future `baton bench` orchestrates the same engine. Stage 27 remains blocked until all slices complete. AC: reproducible manifest-driven runner and report schema; exact-output/hash validation; initial compiler/program/runtime case matrix; recorded development/release baselines; deterministic CI structural checks.
  **Slice status:** Stage 26b — In Progress; Stage 26b Slice 1 — Complete; Stage 26b Slice 2 — Complete; Stage 26b Slice 3 — In Progress; Stage 26b Timing Threshold Review — Next.

### Phase E — Enums, match, errors (Stages 27–29)
- **Stage 27 — Enums + payload cases — Blocked Until Stage 26b Completes.** D6, inline tagged layout, Copy/move classification per payloads. AC: `Shape` example native.
