...
**Formatted I/O — the v1.0 minimal set (future formatted-I/O decision; number assigned when authored).** Doria ships a deliberately small, PHP-familiar formatted-I/O surface in the free-function layer; the broader PHP string-function catalogue (php.net's `ref.strings`) is a post-1.0 expansion, imported case by case under the §9.1 charter with regularized names, never wholesale.
...
- **`sscanf` is deferred, not spelled.** Its shape — runtime-format-determined result arity/types, or by-reference out-parameters — fundamentally fights static typing (Rust has no scanf either, and Doria has no tuples to receive one). v1.0 parsing is `Int::parse`/`Float::parse` returning `?T`, the `str_` functions, and `match`/narrowing; a compile-time-checked scan design may be revisited post-1.0 in the future formatted-I/O decision's follow-up.
...
- **Stage 15 — LLVM release backend (complete).** `--release` through LLVM over the same validated MIR; the durable differential suite triples. AC passed: all finite native examples are identical across interpreter/Cranelift/LLVM and release binaries pass the suite.
...
- **Stage 17 — std::io v0 + formatted I/O.** stdin/stdout/stderr streams, file read/write over `string` lines; the §9 formatted-I/O minimal set: `readline(): ?string` (newline-stripped, `null` at EOF), and compiler-known `sprintf`/`printf` with compile-time-checked literal format strings over the v1.0 specifier subset; `print` rejected with a use-`echo` diagnostic. `Bytes` is deferred to Stage 23: a mutable buffer is a move type, so it belongs after the ownership stages. AC: cat-clone and line-count example programs; echoing-readline loop example; sprintf fixture matrix (`%05d`, `%.2f`, `%x`, width/align) with exact expected output; format-mismatch diagnostic snapshots (`%d` with string arg, wrong arity, non-literal format).
...
- **Stage 46 — std::term v0 (future `std::term` decision; number assigned when authored).** The §9 portable terminal layer: raw-mode enter/leave (restored on panic and on error escaping `main` — a wedged terminal is the classic TUI failure), key/resize event decoding to enums, cursor/style/clear/size, both platform backends landing together per §8.6. May proceed in parallel with Stage 45. AC: an interactive demo (input echo + moving glyph + resize handling) runs from the same source on Linux, macOS, and Windows; Unix CI drives it under a pty harness, Windows CI under a ConPTY harness; zero escape sequences appear in the demo's source.
