...
- **Stage 43 — Engine profile.** declare-based overflow relaxation for audited modules; arena allocator hooks in doria-rt; benchmark suite (criterion-style) vs C/Rust baselines for drop-glue, `SharedReference<T>`, and collection hot paths. It continues and broadens Stage 36a's already-enforced stream performance/memory gate; it does not defer the initial stream benchmarks or their regression thresholds until this stage.
