...
- **Stage 20a — Copy-scalar default arguments.** Decision 0086 completes native calls that omit trailing const-evaluable fixed-width integer, float, or bool arguments. One caller-side splice serves free functions, instance methods, static methods, and constructors; writable Copy-scalar parameters remain valid. Const-literal string defaults are a committed follow-up, while move-type and `take` defaults remain ownership work. AC: omitted and explicit defaults match across the interpreter, Cranelift, and LLVM, including promoted constructor properties and class-constant defaults.
