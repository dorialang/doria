...
- Decision 0119: checked Errors, Error values, `throws` effects, propagation, and runtime outcomes — `Error` is a compiler-known explicit-conformance Move interface; `throw` transfers ownership; `try`/`catch` coverage is checked; callable signatures carry semantic effect sets; checked propagation reuses structured finalizer regions; Slice 1 implements checking and an execution boundary, Slice 2 owns execution, and Slice 3 owns I/O migration plus R1000.
...
- **Unified Doria Diagnostic Presentation And Runtime Outcome Foundation —
  ...
  presentation; Stage 29 Slice 1 checking is complete, while propagation and
  runtime rendering remain in Slices 2 and 3.
...
- **Stage 29 — Checked errors end-to-end — In Progress.** Decision 0119 settles the complete model. **Slice 1 — Complete:** dedicated `try`/`catch`/`throw`/`throws` grammar; backend-independent AST/HIR; compiler-known explicit `Error` conformance with readonly external `string $message`; source-ordered semantic effect sets; exact concrete and `Error` catch coverage; catch-or-declare checking; throw ownership; constructor/property/static/constant/destructor/finally effects; diagnostics; tooling; and one shared pre-MIR execution boundary. `check`, `ast`, and `hir` accept valid checked-error source while MIR, native/debug/PHP emission, and run stop once with the Slice 2 diagnostic. Nonthrowing programs remain executable. **Slice 2 — Next:** two-word carrier, descriptor and hidden first-origin storage, status/out-slot ABI, checked MIR, propagation, catch dispatch, rethrow, finalizer reuse, failed-construction cleanup, and interpreter/Cranelift/LLVM/PHP transport parity. **Slice 3 — Pending:** the record-0075 I/O migration, canonical `Doria\Std\Io` Error types, R1000 shared runtime outcome, status 70, and standalone/tooling closure. Checked errors clean up but never roll back side effects; panic remains fatal status 101 and cleanup-free; the ordinary closed-standard-stream status-0 rule remains permanent and never throws. Stage 30 stays blocked until Stage 29 completes. AC for Slice 1: conformance, effect, catch, ownership, construction, and scope matrices; valid AST/HIR review; one backend boundary; nonthrowing regression; compiler/LSP/editor/website pin agreement.
