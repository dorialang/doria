...
| D19 | Naming charter & the built-in/userland boundary         | **Only built-in free functions use `snake_case`**, for example `get_time()` and `str_starts_with()`. Userland free functions and all member-style APIs use `camelCase`: instance methods, static methods, companion/type APIs, properties, parameters, and named arguments, including `Int::wrappingAdd()` and `$s->isEmpty()`. Types and enum cases use `PascalCase`, constants use `SCREAMING_SNAKE_CASE`, and type parameters use single Pascal capitals. `__construct` and `__destruct` retain their inherited PHP spellings. §9.1 is checkable API law                                                                                         |
...
- **core** (no I/O, always available): `Int`/`Int8`.../`Float`/`Bool`/`String` companion APIs (`Int::parse`, `Int::toFloat`, `Int::wrappingAdd`, ...), `Option`-free nullable helpers, `Cloneable` (Stage 35 interface contract), `Shared<T>`/`Weak<T>`/`SharedMut<T>`, `Comparable<T>`, `Equatable<T>`, `Hashable`, `Displayable`, `Error`, `Iterable<T>`/`Iterator<T>` (Stage 35 user conformance; collections use compiler-internal iteration earlier), range types, `math` basics, and a PHP-familiar built-in free-function layer (`get_time`, `str_starts_with`, `str_case_compare`, ...) that wraps the camelCase method/companion surface — regularized names only, never PHP's fused spellings.
...
### 9.1 Naming charter and the built-in free-function boundary (D19)

PHP's standard library is the cautionary tale this charter exists to prevent: fused spellings, inconsistent word boundaries, and argument orders that vary between related APIs. Doria chooses casing by API category rather than making every built-in symbol look alike:

- **Built-in free functions use `snake_case`**: `get_time()`, `str_starts_with()`, `str_case_compare()`.
- **Userland free functions use `camelCase`**: `loadUser()`, `normalizeTitle()`.
- **All member-style APIs use `camelCase`**, whether built in or user-authored: instance methods, static methods, companion/type APIs, properties, parameters, and named arguments. Examples include `Int::wrappingAdd()`, `$s->isEmpty()`, `$message->tenantId`, `$message->retryAfter(seconds: 30)`, and `$repository->findById($id)`.
- **Types and cases use `PascalCase`**: classes, interfaces, traits, enums, and enum cases.
- **Constants use `SCREAMING_SNAKE_CASE`**.
- **Type parameters use single Pascal capitals** such as `T`, `K`, and `V`.
- **Magic methods keep inherited PHP spelling**: `__construct` and `__destruct` are deliberate exceptions.
...
The rest of the charter keeps API families predictable:

- **No contractions**: `length` not `len`, `toString` not `strval`, and, for the built-in free-function family, **`str_case_compare`, not `strcasecmp`**. Familiar abbreviations are limited to forms more recognizable than their expansions (`str`, `id`, `min`, `max`, `io`, `http`, `json`, `utf8`).
- **Symmetric pairs**: member conversions use `toX` / `fromX` / `tryFromX`; lifecycle verbs pair predictably (`open`/`close`, `push`/`pop`, `add`/`remove`, `intoRaw`/`fromRaw`).
- **Predicates read as questions**: method predicates use camelCase forms such as `isEmpty()`, `hasValue()`, and `canRetry()`. Noun-shaped data remains a property under SPEC §6.
...
- **One name per concept** across modules: it is `count` everywhere (never `size` or `length` for collections), and `contains` everywhere.

The casing boundary applies specifically to free functions: snake_case signals a Doria built-in free function, while camelCase signals a userland free function. Members remain camelCase regardless of whether their implementation belongs to the standard library or user code; the receiver/type communicates ownership. Enforcement:

- Documentation, SPEC examples, `baton new` templates, LSP snippets, and generated members use the category-appropriate casing. Implementations of built-in interfaces therefore use camelCase member names such as `compareTo` and `toString`.
- A default-on lint warns when a userland free function or any non-magic member is declared in snake_case. The lint is silenceable per declaration and per module.
- `function_exists("name")` is a compile-time predicate usable in top-level `if` to conditionally declare a function. Guarded declarations may adopt a built-in free function's snake_case name when deliberately supplying a compatibility polyfill; outside such a guard, userland free functions stay camelCase. `function_exists` is const-evaluated; there is no runtime symbol table.
- Generated PHP FFI stubs mirror the exported Doria class's camelCase member surface, so a `#[PhpExport]` class lands in PHP looking like idiomatic PSR code.

Every stdlib decision record cites this charter, and `baton fmt` plus the stdlib lint enforce it mechanically.

Stdlib API style follows SPEC §6's nouns-are-properties rule. The collection method surface gets its own decision record: List uses `add`, `insertAt`, `removeAt`, `contains`, `count` property, `isEmpty()`, and `map`/`filter`/`reduce` after closures land; Dictionary uses `get` returning `?V`, `set`, `remove`, `has`, `keys`, and `values`; Set uses `add`, `remove`, `has`, `union`, and `intersect`. The typed-array `T[]` surface (`length` property, iteration, slicing) is decided alongside in record 0070, and future named collections (`Queue<T>`, `Stack<T>`, ...) follow the same charter.
...
- 0040 panics and overflow policy (D4, §3.6): `docs/decisions/0040-panics-and-overflow-policy.md`.
- 0041 integer division, modulo, shifts, and bitwise operators (D14): `docs/decisions/0041-integer-division-modulo-shifts-and-bitwise.md`.
- 0042 explicit numeric conversions (D15): `docs/decisions/0042-explicit-numeric-conversions.md`.
- 0043 MIR + interpreter oracle (§8.1-8.2): `docs/decisions/0043-stage-11-mir-and-interpreter-oracle.md`.
- 0044 doria-rt internal ABI (D18): `docs/decisions/0044-doria-rt-internal-abi.md`.
- 0069 dynamic boundary types (D21): `docs/decisions/0069-dynamic-boundary-types.md`.
- 0071 Stage 12 general control flow and dataflow: `docs/decisions/0071-stage-12-general-control-flow-and-dataflow.md`.

Remaining records to author, numbering continues from 0037:

0038 ownership and move semantics (D1, D3) · 0039 borrowing rules, elision, and the borrow checker (D2) · 0045 runtime strings/Bytes (D16) · 0046 nullable types & narrowing (D5) · 0047 enums & payload cases (D6) · 0048 match (D7) · 0049 checked errors full semantics incl. errors escaping `main` (D8, §5) · 0050 generics & monomorphization (D9) · 0051 collections runtime & API surface (§9) · 0052 compiler-internal iteration machinery · 0053 inheritance/open/override (D17) · 0054 interfaces, traits, Cloneable, and public Iterable conformance · 0055 property hooks · 0056 statics & const evaluation · 0057 closures (D10) · 0058 namespaces implementation notes (elaborating 0028) · 0059 attributes & compile-time evaluation policy · 0060 Baton manifest & test convention · 0061 unsafe/FFI (D12) · 0062 php-lib bridge (D13c) · 0063 async & Shareable (D11) · 0064 when grammar · 0065 SIMD/engine intrinsics direction · 0066 `Shared<T>` / `Weak<T>` / `SharedMut<T>` · 0067 naming charter, built-in/userland boundary, and `function_exists` (D19) · 0068 explicit typing discipline (D20) · 0070 typed arrays `T[]`, collection literals and their defaults, and the rejection of `array` (D22).
...
- **Stage 11 — MIR + interpreter oracle (complete).** MIR owns all accepted Stage ≤10 executable lowering; the debug interpreter and Cranelift consume the same MIR; the manifest-driven differential harness verifies exact stdout and process status; and no Stage 7-10 smoke-module code remains.
- **Stage 12 — General control flow + runtime foundation (complete).** The reusable source CFG/dataflow framework provides path-sensitive reachable-fallthrough return checking. Supported nested control flow permits `return` anywhere, recursion and mutual recursion are enabled, ordinary interpretation has no artificial block/call-depth caps, and native compilation has no execution preflight. The allocation-free `crates/doria-rt` owns native entry, exact output, generated Doria stack traversal, abort-only explicit panic, checked `int` overflow panic, and invalid-process-status panic. The durable differential matrix compares stdout, stderr, and status.
- **Stage 13 — Full integer family + operators (complete).** All eight fixed-width integer types share one canonical compiler model; `int` and `int64` are one signed 64-bit type. Contextual literals, checked arithmetic, division/remainder, shifts, bitwise operators, unary negation, increments, compound assignments, and checked companion `from` conversions flow through typed MIR, the interpreter, and Cranelift. The durable differential suite covers narrow signed/unsigned ABI values, `uint64` boundary transport, exact panic stderr/stack traces/status 101, and a manifest-complete arithmetic torture set. PHP rejects precise integer shapes it cannot preserve rather than changing Doria semantics.
- **Stage 14 — Floats + bool runtime (complete).** Canonical IEEE binary32/binary64 values and runtime bools share the MIR scalar path across locals, parameters, returns, calls, assignments, and branches. Arithmetic, comparisons, special values, short-circuit bool expressions, `Int::toFloat`, checked `Float::toInt`, Cranelift ABI lowering, PHP boundaries, and durable differential fixtures pass the full repository validation gate. Stage 15 is next.
