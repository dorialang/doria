...
- **Interactive Line-Input Amendment — Implemented.** Amends decision 0074's
  ...
  is complete, and decision 0110 accepts the stream architecture and performance contract. Stage 26, Stage 26a, and Stage 26b are complete; the performance `Measurement Status: Pending Available Runner` is non-blocking, Decision 0113 Slices 1-3 are complete, Slice 4 is next, and Stage 27 is sequenced after Decision 0113 with no performance-evidence dependency.

  **Current planning checkpoint:** Stage 25a — Complete; PHP Stream And I/O Completeness Audit — Implemented; Andrew’s Stream API Completeness Review — Complete; Stream Architecture And Performance Decision — Accepted (decision 0110); Stage 26 — Complete; Stage 26a — Complete; Stage 26b — Complete, All Three Slices Complete (decision 0112); Measurement Status: Pending Available Runner; Decision 0113 Slice 1 — Complete; Decision 0113 Slice 2 — Complete; Decision 0113 Slice 3 — Complete; Decision 0113 Slice 4 — Next; Stage 27 — Sequenced After Decision 0113, No Performance-Evidence Dependency; Stage 35a — Scheduled; Stage 36a — Scheduled; Stage 36a Public Spellings — Deferred; Stage 36a — Not Implemented.
...
- **Decision 0113 — Collection Surface Completion — Slices 1-3 Complete.** Slice 2 adds one compiler-owned receiver-aware suggestion table, structured E0521 fixes, a dedicated property-invoked-as-method diagnostic, operation-specific equality diagnostics, bracket-literal guidance for withdrawn `List::from` / `Dictionary::from`, and accepted-pending diagnostics that stop before MIR. Slice 3 executes first-position `List::indexOf`, first-match writable `List::remove`, O(n) map `containsValue`, and borrowed O(1) set endpoint properties through shared validated MIR and all native backends; the ordered-family PHP helpers preserve the same target-state semantics. Slice 4 `clear()` is next and remains the only Decision 0113 member carrying E0559.

### Phase E — Enums, match, errors (Stages 27–29)
- **Stage 27 — Enums + payload cases — Sequenced After Decision 0113; No Performance-Evidence Dependency.** D6, inline tagged layout, Copy/move classification per payloads. AC: `Shape` example native.
