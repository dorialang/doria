...
- **Interactive Line-Input Amendment — Implemented.** Amends decision 0074's
  ...
  **Current planning checkpoint:**
  ...
  - **Native Testing Foundation:** Slice 1 — Complete under Decision 0129; Slice 2 — Next; Foundation — In Progress, Not Complete.
...
- **Stage 30 Closure Authority — Accepted And Implemented; Stage 30 — Complete.** Decision 0121 elaborates Decision 0120 and applies Decision 0119. Function values are Move-only structural values with independent value ownership, readonly/writable/once invocation, and per-argument ownership. Function types preserve checked effects; arbitrary callable expressions invoke positionally after one callee evaluation; named and bound callable references remain deferred. The subsection order below is the accepted dependency order.
  ...
  - **Continuation:** Stages 31 through 33 and Phase F are complete; Native Testing Foundation Slice 1 is complete and Slice 2 is next. The debug interpreter remains the semantic oracle; Cranelift, LLVM, and PHP compatibility consume the same validated closure authority for their supported surfaces.
...
- **Stage 33 — Baton package and dependency workflow in the disposable PHP UX bootstrap — Complete.** Decisions 0118, 0124, 0126, 0127, and 0128. Stage 33 completes and exercises the durable Baton product contract in `dorialang/baton-php`; it does not promote PHP to Baton's permanent implementation language and does not satisfy the Doria-native transition.
  ...
  - **Phase F — Complete.**
  - **Continuation:** Native Testing Foundation Slice 1 is complete, Slice 2 is next, and Stage 34 single class inheritance waits for the foundation to complete. Decision 0124's Pre-Stage-45 Doria-native Baton transition remains scheduled and mandatory before the first unsuffixed `2026.03.1` release.
...
- **Pre-Stage-34 Native Testing Foundation — In Progress, Not Complete.** Decision 0129. Compiler-owned test declarations and metadata extend the completed Stage 33 runner without reopening Phase F.
  ...
  - **Slice 2 — Next:** Implements `expect`, `fail`, ephemeral expectations, negation, scalar/string/null/bool/ordered assertions, `AssertionError`, the `TestAssertion` effect, structured outcomes, cleanup, and interpreter/Cranelift/LLVM/PHP parity. These identities remain a single stage-named unsupported boundary until this slice lands.
  ...
  - **Status:** Stage 33 — Complete; Phase F — Complete; Native Testing Foundation Slice 1 — Complete; Native Testing Foundation Slice 2 — Next; Native Testing Foundation — In Progress, Not Complete; Stage 34 Single Class Inheritance — Blocked Until The Foundation Completes.
