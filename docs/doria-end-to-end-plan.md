...
- **Canonical member-casing examples (normative; preserve these spellings):** `Int::wrappingAdd`, `$s->isEmpty`, `$request->retryAfter`, `$repo->findById`, and `$account->tenantId`. These examples deliberately cover companion APIs, built-in members, and userland members so naming-authority checks can detect a regression to stale snake_case member spellings.
