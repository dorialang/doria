...
- **Interactive Line-Input Amendment — Implemented.** Amends decision 0074's
  ...
  is complete, and decision 0110 accepts the stream architecture and performance contract. Stage 26 and Stage 26a are complete; Stage 26b is in progress with decision 0112 accepted and Slice 1 complete, Slice 2 next, and Stage 27 blocked until Stage 26b completes.

  **Current planning checkpoint:** Stage 25a — Complete; PHP Stream And I/O Completeness Audit — Implemented; Andrew’s Stream API Completeness Review — Complete; Stream Architecture And Performance Decision — Accepted (decision 0110); Stage 26 — Complete; Stage 26a — Complete; Stage 26b — In Progress, Slice 1 Complete, Slice 2 Next (decision 0112); Stage 27 — Blocked Until Stage 26b Completes; Stage 35a — Scheduled; Stage 36a — Scheduled; Stage 36a Public Spellings — Deferred; Stage 36a — Not Implemented.
...
- **Stage 26b — Performance Baseline Foundation — In Progress; Slice 1 Complete, Slice 2 Next.** Decision 0112 accepts one repository-owned benchmark system that later stages extend; this is measurement infrastructure and accepted baselines, not an unlimited optimization campaign. Slice 1 supplies the strict manifest and report schema, committed exact-output authority, round-robin raw sampling, explicit compiler/driver provenance, opt-in compiler phase reporting, and matched `call_overhead`, `checked_arith`, and `element_access` diagnostic pairs. Slice 2 records the initial controlled compiler/program/runtime matrix; Slice 3 accepts deterministic regression baselines and the stage-completion workflow. It has separate compiler-performance, generated-program-performance, and runtime-subsystem tracks. Compiler measurements include parse, semantic, MIR, Cranelift, LLVM, link, total compile time, peak RSS, and specialization count. Program measurements distinguish cold startup from hot throughput and record latency, wall/user/system time, peak RSS, allocations where available, binary/stripped/compressed size where useful, and correctness output/hash. Runtime cases begin with implemented strings, objects, methods, ownership, collections, generics, and shared ownership; later streams, async, and FFI join at their owning stages. Cranelift targets responsive development compilation/linking with acceptable runtime performance; LLVM targets release runtime, memory, optimization, and artifact size; the interpreter remains the semantic oracle, not a native performance competitor. Initial cases use only implemented language features and compare first with equivalent C, Rust, and PHP sources. The repository-owned runner records versions, flags, machine, inputs, runner identity, and raw evidence; correctness passes before timing; unfavorable results remain visible; shared CI uses deterministic structural checks while controlled runners own timing thresholds. A future `baton bench` orchestrates the same engine. Stage 27 remains blocked until all slices complete. AC: reproducible manifest-driven runner and report schema; exact-output/hash validation; initial compiler/program/runtime case matrix; recorded development/release baselines; deterministic CI structural checks.
  **Slice status:** Stage 26b — In Progress; Stage 26b Slice 1 — Complete; Stage 26b Slice 2 — Next.
