...
- `SharedReference<T>` / `WeakReference<T>` / `WritableSharedReference<T>` / `WritableWeakReference<T>` and the `ReadonlySharedReferenceAccess<T>` / `WritableSharedReferenceAccess<T>` access objects (authored at Stage 25a).
