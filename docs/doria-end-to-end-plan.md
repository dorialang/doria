...
- **`foreach` borrows elements**: `foreach ($users as User $user)` takes a readonly borrow per iteration; `foreach ($users as writable User $user)` takes exclusive borrows for in-place mutation, requiring the collection binding itself to be writable. Decision 0133 requires the element type at every foreach binding.
...
function max<T implements Comparable<T>>(take T $a, take T $b): T
{
...
- **Manifest and source inventory (decisions 0117, 0118, and 0124):** human-edited schema 2 `Baton.toml` records `vendor/package` identity, SemVer version, edition, targets, `[autoload.namespaces]`, `[autoload-dev.namespaces]`, dependencies, development dependencies, and processors. The disposable PHP bootstrap's schema 1 remains one explicit-entry binary with no autoload, dependency, lockfile, or workspace semantics until Stage 33 implements schema 2 and freezes the exercised product contract. Stage 33 does not make PHP the permanent Baton implementation; the binding Pre-Stage-45 transition ports that contract to the clean Doria-native `dorialang/baton` repository and cuts public distributions over to the native executable.
...
- Decision 0121: closure function types, capture semantics, and execution model — Move-only structural function values separate value ownership, readonly/writable/once invocation, and argument ownership; function types carry checked effects; arbitrary callable expressions are invoked positionally; `$this` is captured explicitly; binding identities, creation-time acquisition, lifetime/escape, a lean two-word carrier, private physical field reordering, E0641 retirement, and the completed Stage 30a through Stage 30h sequence are fixed. Stage 30g adds `map`, Copy-or-Cloneable `filter`, and writable-accumulator `reduce` to `List<T>` only; readonly callbacks are borrowed readonly, while writable callbacks are borrowed exclusively. Stage 30h closes every accepted compiler/backend route and makes E0641 historical and reserved. Stages 31 through 33 and Phase F are complete; the Native Testing Foundation now precedes Stage 34.
...
- Decision 0124: Baton bootstrap, Doria-native transition, and toolchain release gate — Stage 33 completes and validates the durable product contract in the disposable PHP bootstrap; a mandatory Pre-Stage-45 beat ports that behavior into the clean `dorialang/baton` repository, parity-gates the two implementations, removes PHP from production archives, and blocks the unsuffixed `2026.03.1` release until cutover.
- Decision 0125: typed attributes, const-evaluated metadata, and processor protocol — Stage 32 implements source-preserving `#[...]` syntax, typed attribute classes and applications, one bounded constant-evaluation tier, canonical metadata documents, and a strict versioned processor request/response protocol. Attributes are compiler/tooling metadata, never runtime reflection or arbitrary compile-time execution; processor orchestration remains Stage 33 work.
- Decision 0126: Baton schema 2 local package, target, and selection spellings — Stage 33 Slice 1 implements exact schema 1 compatibility, strict schema 2, explicit local publishability and `local/<name>` compiler identity, binary/library targets, selectors, deterministic autoload discovery, and target-scoped plans and receipts in the disposable PHP bootstrap.
- Decision 0127: Baton dependency resolution, lockfile, cache, and offline semantics — Stage 33 Slice 2 implements strict normal path/Git dependencies, SemVer validation, one-version and source-substitution diagnostics, deterministic JSON `Baton.lock`, exact locked Git commits, dependency commands, a global immutable Git cache, centralized offline resolution, multi-package build plans, and receipt lock/path identities in the disposable PHP bootstrap.
- Decision 0128: Baton workspaces, development, tests, processors, and project inventory — Stage 33 Slice 3 implements canonical source transport/locator spellings, workspace lock schema 2, development graphs, compiler-metadata-driven tests, one-pass processors, generated-source ownership, non-executing graph inspection, incremental project inventory, and asynchronous language-server consumption. Stage 33 and Phase F are complete.
- Decision 0129: native testing foundation, behavioral DSL, fluent expectations, and assertion outcomes — Slice 1 implements exact compiler-owned `Doria\Std\Test` `describe`/`it`/`test` declarations, const descriptions, nested suites, deterministic generated callables, unified `#[Test]` metadata schema 3, Baton discovery/dispatch, and compiler-fact-based tooling without runtime registration or source parsing. Slice 2 implements `expect`/`fail`, assertion semantics, and structured outcomes. Slice 3 completes collection/Error expectations, bounded differences, strict Baton classification and hierarchy reporting, and final compiler-fact-based tooling. The foundation is complete and Stage 34 is complete and Stage 35 Slices 1, 2, 3, and 4 are complete and Slice 5 is next.
- Decision 0130: single class inheritance, open and override, parent construction, and hierarchy dispatch — Stage 34 implements closed-by-default classes, explicit virtual methods and overrides, generic parent types, parent-prefix layout, parent-first construction, derived-first destruction, direct `parent::` calls, implicit upcasts, hierarchy narrowing, checked-Error hierarchy coverage, static descriptors, and direct/virtual backend dispatch without per-object headers or runtime reflection.
- Decision 0131: constructor property overrides and constructor-only parameters — default/internal constructor promotion remains; `override` reuses one exact inherited external property without a child field, store, or drop; `parameter` creates a constructor-only callable input; parent construction stays explicit and E0727 continues to reject actual hiding. The Post-Stage-34 Constructor Parameter Roles Corrective Beat is complete.
...
- **Interactive Line-Input Amendment — Implemented.** Amends decision 0074's
  ...
  is complete, and decision 0110 accepts the stream architecture and performance contract. Stage 26, Stage 26a, Stage 26b, all four Decision 0113 slices, both Stage 27 slices, both Stage 28 slices, both Stage 28a slices, all three Stage 29 slices, the pre-Stage-30 closure grammar slice, Stages 30a through 30h, and the Constructor Writable-Path And Owned-Property Corrective Beat are complete. Decision 0121 accepts and implements the complete Stage 30 closure authority, Decision 0122 accepts the constructor/property correction, Decision 0123 implements ambient canonical I/O effects plus fallible-finalizer precedence, Decision 0124 binds the Stage 33 PHP UX-contract milestone to a later Pre-Stage-45 Doria-native Baton transition and `2026.03.1` release blocker, Decision 0125 implements typed attributes and their strict metadata protocol, Decision 0126 implements Stage 33 Slice 1, Decision 0127 implements Stage 33 Slice 2, Decision 0128 implements Stage 33 Slice 3, and Decision 0129 implements Native Testing Foundation Slices 1 through 3. The performance `Measurement Status: Pending Available Runner` remains non-blocking and is not a performance pass. Stages 30 through 33 and Phase F are complete. E0632, E0641, E0671, and E0672 are historical and reserved. The Native Testing Foundation is complete, and Stage 34 single class inheritance is complete and Stage 35 interfaces and traits Slices 1, 2, 3, and 4 are complete and Slice 5 is next.

  **Current planning checkpoint:**

  - **Through Stage 28a:** Stage 25a — Complete; PHP Stream And I/O Completeness Audit — Implemented; Andrew’s Stream API Completeness Review — Complete; Stream Architecture And Performance Decision — Accepted (decision 0110); Stage 26 — Complete; Stage 26a — Complete; Stage 26b — Complete, All Three Slices Complete (decision 0112); Decision 0113 — Complete; Stage 27 — Complete; Decision 0115 — Implemented; Stage 28 — Complete; Decision 0116 — Implemented; Stage 28a — Complete.
  - **Stage 29:** Decision 0119 — Accepted And Implemented; Stage 29 Slice 1 — Complete; Stage 29 Slice 2 — Complete; Corrective Beat: Native Collection Property Initializers — Complete; Stage 29 Slice 3 — Complete; Corrective Beat: Inferred Main Checked Effects — Complete; Stage 29 — Complete.
  - **Stage 30 foundation:** Pre-Stage-30 Grammar Slice — Complete; Stage 30 Closure Authority — Accepted And Implemented; Stage 30a Callable Grammar Completion — Complete; Stage 30b Semantic Function Types And Captures — Complete; Constructor Writable-Path And Owned-Property Corrective Beat — Complete.
  - **Stage 30 execution:** Stage 30c Ownership, Lifetime, And Escape — Complete; Stage 30d Closure HIR/MIR And Interpreter Oracle — Complete; Stage 30e Native Execution — Complete; Stage 30f PHP Compatibility — Complete; Stage 30g List Algorithms — Complete; Stage 30h Cross-Repository Closure — Complete; Stage 30 — Complete; E0641 — Historical And Reserved.
  - **Stage 31:** Stage 31 Slice 1 — Complete; Stage 31 Slice 2 — Complete; Stage 31 — Complete; E0671 — Historical And Reserved; E0672 — Historical And Reserved.
  - **Stage 32:** Ambient I/O And Fallible Finalizer Corrective Beat — Complete; Pre-Stage-32 Ambient I/O And Fallible Finalizer Corrective Beat — Complete (decision 0123); E0632 — Historical And Reserved; Decision 0125 — Accepted And Implemented; Stage 32 — Complete.
  - **Stage 33:** Slice 1 — Complete under Decision 0126; Slice 2 — Complete under Decision 0127; Slice 3 — Complete under Decision 0128; Stage 33 — Complete; Phase F — Complete.
  - **Native Testing Foundation:** Slice 1 — Complete under Decision 0129; Slice 2 — Complete under Decision 0129; Slice 3 — Complete under Decision 0129; Foundation — Complete.
  - **Stage 34:** Single Class Inheritance — Complete under Decision 0130.
  ...
  - **Scheduled:** Stage 35a — Scheduled; Stage 36 Property Hooks — Scheduled; Stage 36a — Scheduled; Stage 36a Public Spellings — Deferred; Stage 36a — Not Implemented; Pre-Stage-45 Doria-Native Baton Transition — Scheduled.
...
- **Stage 29 — Checked errors end-to-end — Complete.** Decision 0119 settles and implements the complete model.
  ...
  - **Continuation:** The pre-Stage-30 closure grammar slice, Decision 0121 authority beat, Stages 30a through 30h, and the Decision 0122 corrective beat are complete; Stages 30 through 33, Phase F, and Native Testing Foundation Slices 1 through 3 are complete. Stage 34 is complete and Stage 35 Slices 1, 2, 3, and 4 are complete and Slice 5 is next.
...
- **Stage 30 Closure Authority — Accepted And Implemented; Stage 30 — Complete.** Decision 0121 elaborates Decision 0120 and applies Decision 0119. Function values are Move-only structural values with independent value ownership, readonly/writable/once invocation, and per-argument ownership. Function types preserve checked effects; arbitrary callable expressions invoke positionally after one callee evaluation; named and bound callable references remain deferred. The subsection order below is the accepted dependency order.
  ...
  - **Stage 30g — List Algorithms — Complete:** Adds explicit semantic/HIR plans and one validated traversal CFG for `List<T>::map`, Copy-or-Cloneable `filter`, and writable-accumulator `reduce`; all supported backends preserve exact callback effects, readonly source traversal, and checked partial-state cleanup.
  ...
  - **Continuation:** Stages 31 through 33, Phase F, and Native Testing Foundation Slices 1 through 3 are complete; Stage 34 is complete and Stage 35 Slices 1, 2, 3, and 4 are complete and Slice 5 is next. The debug interpreter remains the semantic oracle; Cranelift, LLVM, and PHP compatibility consume the same validated closure authority for their supported surfaces.
  ...
  14. `List<T>` alone receives `map`, Copy-or-Cloneable preserving `filter`, and writable-accumulator `reduce` in Stage 30g.
...
- **Stage 31 — Namespaces and package compilation graph — Complete.**
  ...
  - **Slice 2 — Complete:** schema-1 versioned JSON build plans, explicit package/source inventories, filesystem and in-memory source providers, package containment and exact-case checks, include-once graph closure, complete declaration indexing, direct and development dependency visibility, package-wide `internal`, strict hybrid layout, declaration-only non-entry sources, multi-source diagnostics/HIR/MIR/runtime identities, debug/Cranelift/LLVM/PHP execution, and compiler-owned in-memory fingerprints and invalidation. E0671 and E0672 have no live complete-graph route and remain historical and reserved.
- **Pre-Stage-32 Corrective Beat — Ambient canonical I/O effects and fallible-finalizer precedence — Complete.** Decision 0123 makes exactly `Doria\Std\Io\IoError` and `Doria\Std\Io\InvalidUtf8Error` ambient at the source boundary while retaining their exact HIR/MIR identity, checked transport, catchability, cleanup, and R1000 outcome. Required nonambient effects remain explicit. Fallible finalizers may propagate checked Errors; same-try sibling catches do not catch finalizer failures, finalizer-local and outer catches may, and a finalizer failure supersedes any pending structured exit with exact-once cleanup. E0632 has no live route and is historical and reserved. AC: direct and indirect calls, structural callable identity, nested finalizers, all structured exits, owned payload replacement, interpreter/Cranelift/LLVM/PHP parity, and LSP projections agree.
- **Stage 32 — Typed attributes and const-evaluated metadata — Complete.** Decision 0125.
  - **Language surface:** Adjacent `#[...]` groups preserve qualified attribute names, positional and named arguments, commas, grouping, attachment targets, and exact spans. Attribute classes use ordinary class declarations marked with compiler-known `#[Attribute]`; `#[Test]` and `#[PHPExport]` are compiler-known metadata applications.
  - **Semantic model:** Attribute names use the Stage 31 canonical resolver, including package visibility and dependency boundaries. Applications bind through the shared argument model, validate target roles and repeatability, and evaluate only bounded constant-tier values. Promoted constructor parameters retain one application with both parameter and promoted-property target roles.
  - **Metadata boundary:** Canonical typed metadata flows through semantic information and HIR but does not enter MIR, object layout, ABI, or runtime reflection. `doriac metadata` emits deterministic source/build-plan metadata without executing processors.
  - **Processor protocol:** A strict versioned JSON request/response contract validates schema identity, provenance, diagnostic structure, generated-file hashes, normalized relative paths, collisions, and handwritten-source protection. Stage 32 validates this protocol but performs no processor discovery or execution.
  - **Acceptance criteria:** Lexer/comment adjacency, parsing and recovery, schemas, target validation, constant evaluation, namespace/package resolution, deterministic metadata, strict protocol negatives, PHP metadata-only behavior, incremental fingerprints, native parity, and compiler/LSP/editor projection are covered. `#[PHPExport]` bridge execution remains Stage 41 work; metadata never grants visibility or runtime behavior.
- **Stage 33 — Baton package and dependency workflow in the disposable PHP UX bootstrap — Complete.** Decisions 0118, 0124, 0126, 0127, and 0128. Stage 33 completes and exercises the durable Baton product contract in `dorialang/baton-php`; it does not promote PHP to Baton's permanent implementation language and does not satisfy the Doria-native transition.
  - **Slice 1 — Complete:** strict schema 2 parsing with exact schema 1 compatibility; explicit local/scoped package identity; single-binary shorthand and explicit binary/library targets; `--binary`/`--library` selection; autoload/autoload-dev simple and advanced discovery; main/development/generated input scopes; deterministic contained, case-correct inventories; compiler build-plan schema 1 emission; target-scoped schema-2 layouts; and receipts. Library builds check the plan and record no artifact.
  - **Slice 2 — Complete:** normal path and Git dependencies; exact source descriptors and Git selectors; strict SemVer validation; one-version, source-substitution, and cycle diagnostics with complete chains; deterministic strict JSON `Baton.lock`; exact locked installs; selected/full updates; `install`/`add`/`remove`/`update`/`fetch`; centralized offline policy; global content-addressed Git mirrors and immutable exact checkouts; live path packages; dependency-aware check/build/run; multi-package compiler plans; and receipt lock/path fingerprints.
  - **Slice 3 — Complete:** Decision 0128 implements canonical dependency source descriptors, workspaces, development dependencies, graph commands (`tree` and `why`), incremental inventory, `baton test`, explicit processor registration, generated-source orchestration, asynchronous language-server project inventory, and Phase F closure. Stage 32 supplies typed metadata and processor protocol version 1; it does not own automatic package orchestration. AC: `baton new game && baton test` green; locked offline workspace build; direct-dependency visibility and conflict diagnostics; no implicit build scripts.
  - **Phase F — Complete.**
  - **Continuation:** Native Testing Foundation Slices 1 through 3 are complete and Stage 34 single class inheritance is complete and Stage 35 interfaces and traits Slices 1, 2, 3, and 4 are complete and Slice 5 is next. Decision 0124's Pre-Stage-45 Doria-native Baton transition remains scheduled and mandatory before the first unsuffixed `2026.03.1` release.

- **Pre-Stage-34 Native Testing Foundation — Complete.** Decision 0129. Compiler-owned test declarations, expectations, outcomes, Baton reporting, and tooling extend the completed Stage 33 runner without reopening Phase F.
  - **Slice 1 — Complete:** Exact `Doria\Std\Test` `describe`, `it`, and `test` declarations resolve only through compiler-known identities in active development source. Descriptions are const-evaluated; nested suites and tests become stable semantic/HIR facts and deterministic ordinary generated callables; declaration syntax itself enters no MIR and creates no runtime registry. Strict additive metadata schema 3 unifies behavioral tests with `#[Test]` while schemas 1 and 2 remain exact. Baton consumes one test table, filters by display name, dispatches by stable test identity to the compiler-provided callable, preserves process isolation and exact effects, and never parses Doria. The language server consumes compiler test facts and project source scopes; editors remain presentation clients.
  - **Slice 2 — Complete:** Implements `expect`, `fail`, ephemeral expectations, the `not` property, scalar/string/null/bool/ordered assertions, `AssertionError`, the separate `TestAssertion` effect, strict DORIAO4 outcomes, cleanup, interpreter/Cranelift/LLVM/PHP parity, Baton regression integration, and compiler-fact-based official tooling without a runtime expectation object or mandatory success allocation.
  - **Slice 3 — Complete:** Implements collection and dictionary expectations, typed Error expectations, bounded differences, strict Baton reporting/outcome classification, complete compiler-fact-based tooling navigation, documentation examples, and installed-toolchain closure.
  - **Status:** Stage 33 — Complete; Phase F — Complete; Native Testing Foundation Slice 1 — Complete; Native Testing Foundation Slice 2 — Complete; Native Testing Foundation Slice 3 — Complete; Native Testing Foundation — Complete; Stage 34 Single Class Inheritance — Complete; Stage 35 Interfaces And Traits — In Progress; Slices 1, 2, 3, And 4 Complete; Slice 5 Next; Pre-Stage-45 Doria-Native Baton Transition — Scheduled.

### Phase G — OOP completion and hosted I/O foundation (Stages 34–36a)
- **Stage 34 — Single Class Inheritance — Complete.** Decision 0130.
  - **Declarations and hierarchy:** `open` and `override` are reserved, source-preserving modifiers. Qualified and generic `extends` types resolve through the package graph; the compiler builds one cycle-checked hierarchy with invariant substitutions, inherited lookup, declaring-class `internal`, deterministic virtual roots, and exact incremental dependencies.
  - **Construction and ownership:** one complete object uses parent-prefix layout. Construction is root-first, required `parent::__construct(...)` is the child's first source action, failure cleans only initialized state, destruction is derived-first, and lifecycle-phase dispatch cannot reach uninitialized or destroyed child state.
  - **Representation and dispatch:** closed exact values remain one word; open static types use a two-word data/descriptor carrier. Descriptors and vtables are static per concrete specialization, upcasts allocate nothing, `parent::` is direct, genuine base-typed calls use a constant virtual slot, and proven exact LLVM calls devirtualize.
  - **Type integration:** central assignment compatibility handles upcasts across locals, calls, returns, properties, nullable values, collections, shared ownership, `mixed`, closures, and checked Error transport. `is`, match type patterns, catches, and typed `toThrow` inspectors are hierarchy-aware while preserving ownership and narrowing proof.
  - **Backends and tooling:** shared validated MIR drives the interpreter, Cranelift, LLVM, and PHP compatibility. Metadata schemas 1 through 3 and processor protocol 1 remain exact; private hierarchy layout does not become reflection. Official language-server tooling consumes compiler-owned hierarchy facts.
  - **Acceptance:** durable fixtures cover closed/open classes, three-level and generic hierarchies, parent/default/static calls, construction and destruction, virtual checked/ambient/test effects, upcasts and narrowing, shared/collection/`mixed` transport, Error catches and assertions, lifecycle dispatch, exact-call devirtualization, and cleanup parity across enabled backends.
- **Post-Stage-34 Constructor Parameter Roles Corrective Beat — Complete.** Decision 0131 preserves default and internal promotion, adds `override` constructor parameters that reuse one inherited external property family without adding a field or automatic write, and adds `parameter` constructor-only inputs that remain callable arguments but never object state. The parent call remains explicit and first when required; actual hiding remains E0727. AC: exact source roles/spans; one root field across generic and multilevel inheritance; ordinary ownership for constructor-only and override inputs; no duplicate initialization/drop; attribute roles and metadata schemas unchanged; interpreter, Cranelift, LLVM, and PHP parity; compiler-owned LSP presentation and conservative rename.

- **Post-Stage-34 Indexed Foreach And Scalar Display Corrective Beat — Complete.** Decision 0132 gives `List<T>` and `T[]` a readonly zero-based canonical-`int` first binding, preserves actual keys for Dictionary families, and rejects first bindings on value-only sources before MIR. One compiler-owned semantic plan flows through HIR and validated MIR; sequence elements use positional access, dictionaries retain keyed access, and PHP emits its own collision-safe ordinal rather than adopting host keys. Property-rooted sequences evaluate once and remain borrowed, while existing writable value iteration and structured cleanup are preserved. Interpolation, string-anchored concatenation, and `%s` are clarified as producing ordinary reusable strings without adding scalar assignment coercion, primitive `toString`, or casts. AC: explicitly typed property-rooted, generic, control-flow, ownership, malformed-plan, PHP-shape, and scalar-display fixtures agree across the interpreter, Cranelift, LLVM, and PHP; official tooling consumes compiler facts.

- **Post-Stage-34 Explicit Foreach Binding Types Corrective Beat — Complete.** Decision 0133 requires an authored type on every foreach binding, including range values and both positions of indexed or keyed iteration. `writable` remains an access modifier rather than a type, while Decision 0132's sequence-index and dictionary-key roles remain unchanged. The parser preserves incomplete bindings for recovery; semantic analysis emits E0748 with the exact compiler-resolved type and a local machine-applicable insertion, and invalid source cannot reach HIR or MIR. AC: one-binding, two-binding, range, collection, projection, generic, nullable, and writable forms require explicit types; wrong explicit types retain their precise diagnostics; interpreter, Cranelift, LLVM, and PHP parity remains unchanged for corrected source; tooling forwards the compiler diagnostic and fix.

- **Stage 35 — Interfaces And Traits — In Progress; Slices 1, 2, 3, And 4 Complete; Slice 5 Next.**
  Decision 0134 fixes nominal generic interfaces, multiple interface inheritance,
  exact method compatibility, two-word erased carriers, core value contracts,
  public value-only iteration, shared interface payloads, compile-time generic
  ...
  - **Slice 1 — Complete: Grammar, Graphs, And Conformance.** Complete source-
    ...
    incremental facts, and compiler-fact-based tooling. Slice 2 extends these
    checked facts into interface execution. Slice 4 subsequently completes
    trait composition against those contracts.
  - **Slice 2 — Complete: Interface Runtime And Ownership.** HIR/MIR two-word
    carriers, static vtables, calls/conversions/type tests, dynamic drop,
    nullable/mixed transport, Error/Displayable migration, all six shared-
    ownership families, and interpreter/Cranelift/LLVM/PHP parity.
  - **Slice 3 — Complete: Core Contracts And Public Iteration.** Executes
  ...
  - **Slice 4 — Complete: Trait Composition.** Implements recursive generic
    flattening, bodyless requirements, method adaptations, strict precedence,
    deterministic layout/init/drop/static state, inheritance/interface
    integration, metadata origins, incremental invalidation, backends, and
    tooling. Authored spans and composer-specific effective identities remain
    distinct. E0493 is retired and reserved; no backend composes traits.
  - **Slice 5 — Next: Cross-Repository Closure.** Complete parity,
    malformed-MIR negatives, structural performance checks, installed-toolchain
    UAT, documentation, editor artifacts, website handoff, and Stage 35 status.
  - **Acceptance:** SPEC interface/trait examples execute across supported
    backends; exact/constrained/erased dispatch shapes are proven; a Move-valued
...
- **Pre-Stage-45 Doria-Native Baton Transition — Mandatory.** Decision 0124. Create the clean `dorialang/baton` repository; port the Stage 33 contract to Doria after Stage 36a filesystem/process foundations, Stage 40 native integration, and Stage 44 network/HTTP support; close the remaining environment/path/TOML/JSON/hash/archive readiness gates through reviewed Doria library or package APIs; run the PHP and Doria implementations against one shared behavior suite; then transfer templates, releases, complete toolchain assembly, and the installed `bin/baton` to the Doria repository. Production archives drop the PHAR, Composer, PHP launcher, and private PHP runtime. `dorialang/baton-php` freezes as historical and compatibility reference. AC: every Stage 33 workflow and cross-implementation fixture agrees; native Baton builds, checks, tests, and packages its own repository; clean-machine native-only archives pass on every supported host; the unsuffixed `2026.03.1` release remains blocked until cutover is complete.
- **Stage 45 — Compiler self-hosting start.** After the native Baton transition, port the lexer to Doria as the first self-hosted compiler component (per docs/self-hosting.md), compiled by `doriac`, differentially tested against the Rust lexer. Baton is already a shipping Doria-authored tool at this point.
...
- **1.0 gate** (ships as the first unsuffixed `yyyy.mm.n` release per §11)**:** the Pre-Stage-45 Doria-native Baton transition is complete; the installed Baton comes from `dorialang/baton`; production archives contain no Baton PHP/Composer/private-runtime payload; the shared PHP/Doria compatibility suite and native clean-machine installation matrix pass; spec freeze pass over SPEC.md, diagnostics audit, doria-rt ABI review, differential + fuzz suites green, and the three flagship demos build: a portable TUI game (engine seed — the same binary source running natively on Windows, macOS, and Linux via `Doria\Std\Term`, zero ANSI in user code), a UI component demo, and a PHP app calling a Doria php-lib. At least one reviewed stream-focused demonstration — streaming file copy, a non-blocking child-process pipeline, or a readiness-driven multi-stream program — must also pass; the exact source is chosen after the decision-0110 public spellings are finalized. The unsuffixed `2026.03.1` release cannot ship before every native-Baton cutover criterion passes.

### Post-1.0 scheduled work
- **Stage 48 — Typed multi-value transport and semantics-preserving Extract Method refactoring.** Begin with a language decision for heterogeneous multi-value returns and destructuring; do not repurpose homogeneous `List<T>`, `mixed`, grouped local declarations, generated result classes, or hidden writable output parameters as a tooling-only transport. The decision must settle source spelling, static types, evaluation and destruction order, Copy/move ownership, borrows, partial binding, ignored values, generics, checked effects, MIR representation, backend ABI, and PHP-compat lowering. Decision 0111 remains unchanged: grouped locals continue to initialize independent bindings from one value and are not destructuring. After the language surface is implemented across parser, semantics, MIR, interpreter, Cranelift, LLVM, and PHP compatibility, `dorialang/doria-language-server` adds compiler-backed Extract Method for IntelliJ and VS Code. The refactoring derives a camelCase name from an immediately associated leading comment when possible; identifies read dependencies as parameters; preserves writable/taking ownership and checked effects; returns newly declared or changed values that remain live after the selection; and rewrites the call site with typed destructuring. It is offered only for complete statement selections whose control flow, scope, ownership, and effects can be preserved. AC: zero-, one-, and heterogeneous multi-output extraction fixtures; readonly, writable, take, generic, and checked-error cases; rejection fixtures for escaping `return`/`break`/`continue`, invalid partial statements, and non-preservable borrows; UTF-16-safe edits; identical source transformations in IntelliJ and VS Code; transformed programs pass compiler analysis and differential backend execution.
...
- tuples/product values and multiple-return destructuring (post-1.0 Stage 48; Decision 0111 grouped locals are not destructuring)
