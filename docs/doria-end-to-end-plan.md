...
- **`float32` variants and SIMD**: record 0065 (SIMD direction) treats `std::math` as its first consumer — `Vector3`/`Vector4` over `float32` map directly onto SIMD lanes, so the future math-layout decision and 0065 are made together.
