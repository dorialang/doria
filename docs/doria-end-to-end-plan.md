...
- Name resolution: a compilation invocation takes an explicit source inventory or versioned JSON build plan from Baton; symbols resolve by fully qualified name; unqualified names resolve via current namespace then `use` imports; duplicate fully qualified symbols across files or packages are compile errors identifying both packages and sources. Baton discovers and resolves package inputs, while `doriac` owns every declaration and semantic decision.
...
A closure holding borrows is itself borrow-bound, so it cannot outlive or escape the captured variables' scope — no bespoke escape analysis is needed, the borrow checker rejects it and the diagnostic suggests `take`. Closures are move-typed values with type spelling `function(int): int` in type position; `Callable<...>` alias is not adopted. Decision 0120 owns explicit local capture. `$this` capture remains a bounded Stage 30 decision and is not assigned an invented spelling here.
...
- Decision 0119: checked Errors, Error values, `throws` effects, propagation, and runtime outcomes — `Error` is a compiler-known explicit-conformance Move interface; `throw` transfers ownership; `try`/`catch` coverage is checked; callable signatures carry semantic effect sets; checked propagation reuses structured finalizer regions; Slices 1 and 2 implement checking and handled execution, and Slice 3 owns I/O migration plus R1000.
- Decision 0120: explicit closure capture lists — arrows and anonymous block functions use the same explicit `with` list for every enclosing local dependency; readonly, writable, and taking modes are written; Copy and Move values receive no implicit exception; no-capture closures omit `with`; a pre-Stage-30 grammar slice owns accepted syntax before Stage 30 owns capture checking, callable-effect integration, and accepted closure algorithms; `$this` remains a bounded Stage 30 question.
...
- **Stage 23 — Runtime collections, typed arrays, Bytes, and mixed runtime (three slices).** **Slice 1** implements owned `T[]`, `List`/`Dictionary`/`Set`, contextual collection literals, indexing and indexed read-modify-write, insertion-ordered `foreach` borrows, move-in/removal ownership, and Decision 0100's default member surface in shared MIR and `doria-rt`. Dictionary `keys`/`values` are readonly insertion-ordered `foreach`-only projections, not storable copies or bespoke view values. **Slice 2** implements the `Bytes` move type; explicit copying `uint8[]` conversion; byte indexing, mutation, length, and equality; text `append_file`; whole-file `read_file_bytes`/`write_file_bytes`/`append_file_bytes`; and whole-stream `read_stdin_bytes`/`write_stdout_bytes`/`write_stderr_bytes` per §9 and records 0091/0101. **Slice 3** adds the boxed `dr_mixed` representation and collection element paths that require runtime mixed boxes. Each deferred slice keeps a slice-named semantic diagnostic; no backend placeholder stands in for it. Slice 1 AC: move/borrow fixture matrix over collections and typed arrays; `int[]`/`int[][]` fixed-length fixtures; literal context typing (`int[]` vs `List<int>` from the same literal, uncontextual sequence/keyed defaults); in-place `foreach` and indexed mutation; use-after-move and owned-removal lifetime snapshots; exact interpreter/Cranelift/LLVM parity. Slice 2 AC: non-UTF-8 byte-identical file and standard-stream round trips across all three backends, append/truncate parity, conversion-copy behavior, indexed RMW, equality, and OOB panic. Slice 3 AC: mixed box round trip (int/string/class in, narrowed out), `?mixed`, and heterogeneous mixed collection values across interpreter/Cranelift/LLVM.
...
- **Interactive Line-Input Amendment — Implemented.** Amends decision 0074's
  ...
  is complete, and decision 0110 accepts the stream architecture and performance contract. Stage 26, Stage 26a, Stage 26b, all four Decision 0113 slices, both Stage 27 slices, both Stage 28 slices, and both Stage 28a slices are complete. The performance `Measurement Status: Pending Available Runner` remains non-blocking. Stage 29 is in progress: Slices 1 and 2 and the native collection property initializer corrective beat are complete, and Slice 3 is next.

  **Current planning checkpoint:** Stage 25a — Complete; PHP Stream And I/O Completeness Audit — Implemented; Andrew’s Stream API Completeness Review — Complete; Stream Architecture And Performance Decision — Accepted (decision 0110); Stage 26 — Complete; Stage 26a — Complete; Stage 26b — Complete, All Three Slices Complete (decision 0112); Measurement Status: Pending Available Runner; Decision 0113 Slice 1 — Complete; Decision 0113 Slice 2 — Complete; Decision 0113 Slice 3 — Complete; Decision 0113 Slice 4 — Complete; Decision 0113 — Complete; Stage 27 — Complete; Stage 27 Slice 1 — Complete; Stage 27 Slice 2 — Complete; Decision 0115 — Implemented; Stage 28 — Complete; Stage 28 Slice 1 — Complete; Stage 28 Slice 2 — Complete; Decision 0116 — Implemented; Stage 28a — Complete; Stage 28a Slice 1 — Complete; Stage 28a Slice 2 — Complete; Decision 0119 — Accepted; Stage 29 — In Progress; Stage 29 Slice 1 — Complete; Stage 29 Slice 2 — Complete; Corrective Beat: Native Collection Property Initializers — Complete; Stage 29 Slice 3 — Next; Stage 30 — Blocked Until Stage 29 Completes; Stage 35a — Scheduled; Stage 36a — Scheduled; Stage 36a Public Spellings — Deferred; Stage 36a — Not Implemented.
...
- **Stage 26b — Performance Baseline Foundation — Complete; All Three Slices Complete.** Decision 0112 accepts one repository-owned benchmark system that later stages extend; this is measurement infrastructure and accepted baselines, not an unlimited optimization campaign. Slice 1 supplies the strict manifest/report and compiler-report foundation. Slice 2 supplies deterministic scaling, the compiler/generated/runtime/diagnostic matrix, process-resource and profiler adapters, candidate evidence, and an exact structural baseline without timing thresholds. Slice 3 supplies comparative peers, enforced fairness records, controlled-runner and promotion workflows, deterministic compiler/runtime artifact identity, shared workload profiles, and the exact native acceptance policy. It keeps compiler performance, generated-program performance, runtime-subsystem performance, cold startup, memory, and artifact size as separate dimensions. The interpreter remains a semantic oracle rather than a native competitor. The native runtime status compares each Doria backend with the fastest valid C, C++, or Rust peer and passes only at `ratio <= 1.30`; PHP remains adoption evidence. Correctness and adequacy precede timing, Inconclusive is never Pass, and Doria self-regression thresholds do not weaken cross-language status. A future `baton bench` orchestrates the same engine. AC: reproducible manifest-driven runner and report schema; exact-output/hash validation; deterministic CI structural checks; exact structural baseline; executable controlled-runner and acceptance workflows; exact native acceptance policy.
  All portable obligations are complete: correctness and interpreter/Cranelift/LLVM parity; exact compiler/runtime provenance and deterministic runtime selection; workload scaling and peer-equivalence metadata; the binding `1.30` rule; structural baselines and portable smoke validation; the failure/scheduled-work register; and controlled-runner commands/report schemas. No eligible controlled physical-host Linux sessions exist, so no timing baseline or acceptance matrix is promoted. That evidence is required before release/public performance claims, but it is not a language or compiler stage gate. Docker, WSL, containers, and virtual machines may provide engineering, correctness, workflow-rehearsal, optimization-guidance, and local-regression evidence; their native timing status remains Inconclusive. Controlled Linux timing, verified affinity, Callgrind, DHAT, hardware counters, and cross-platform timing baselines cannot become Stage 26b or Stage 27 closure conditions. Decision 0113 and all four slices are complete; both Stage 27 slices are complete with no performance-evidence dependency.
  **Slice status:** Stage 26b — Complete; Stage 26b Slice 1 — Complete; Stage 26b Slice 2 — Complete; Stage 26b Slice 3 — Complete; Measurement Status: Pending Available Runner.
- **Decision 0113 — Collection Surface Completion — Complete.** Slice 2 adds one compiler-owned receiver-aware suggestion table, structured E0521 fixes, a dedicated property-invoked-as-method diagnostic, operation-specific equality diagnostics, and bracket-literal guidance for withdrawn `List::from` / `Dictionary::from`. Slice 3 executes first-position `List::indexOf`, first-match writable `List::remove`, O(n) map `containsValue`, and borrowed O(1) set endpoint properties. Slice 4 executes writable, zero-argument `clear(): void` in place on all seven named collections through shared validated MIR and exact-once type-aware cleanup; no Decision 0113 member remains routed to E0559.

### Phase E — Enums, match, errors (Stages 27–29)
- **Stage 27 — Enums + payload cases — Complete; No Performance-Evidence Dependency.** Decision 0114. **Slice 1 — Complete:** authority, enum/case grammar, unit and `int`/`string`-backed execution, nominal identity, inline tags, equality, readonly `value`, nullable/`mixed`, constants/statics/defaults, supported collection placement, PHP native enums, and grammar-only payload/`match` preparation. **Slice 2 — Complete:** payload construction with shared positional/named binding, finite central inline layout, recursive Copy/Move classification, case-aware copy/drop/equality glue, aggregate ABI, class/generic/collection storage, nullable and `mixed` transport, Copy constants/defaults, faithful PHP lowering, and exact interpreter/Cranelift/LLVM parity. Generic enums remain deferred and ordinary enum containers allocate no heap object.
- **Stage 28 — match — Complete.** Implemented decision 0115 preserves D7 and delivers both risk-separated slices. **Slice 1 — Complete:** guard-free exhaustive match expressions, unit/payload enum cases, positional payload destructuring and case-only ignore, exact compile-time constants, `null`, exact type-binding narrowing for nullable/`mixed`, strict arm typing, readonly payload ownership, ordered `match (true)`, and full right-associative ternary. **Slice 2 — Complete:** `if` pattern guards with readonly guard views, one-time ordered evaluation and guard-aware coverage/reachability; explicit `match (take $value)` whole-scrutinee consumption; owned selected Move payloads, nullable/`mixed` extraction and exact cleanup; rejected writable patterns; and PHP backend-private exact `mixed` tags. One validated MIR CFG executes across the interpreter, Cranelift, and LLVM; PHP preserves the same checked semantics. AC: durable guard/consumption/destruction parity, malformed-MIR rejection, exact PHP type identity, and coordinated tooling.
- **Stage 28a — Control-flow completion — Complete.** Decision 0116 settles and implements the family. **Slice 1 — Complete:** executable `when` expressions with mandatory `else`, explicit/expected/inferred result typing and nearest-`when` return-to-yield; executable `given` on `if`, `when`, and `while` with setup-before-predicate phases, source-order short-circuiting, condition skipping, and per-condition-check reevaluation for `while`; and executable base `do ... while`. **Slice 2 — Complete:** one backend-neutral finalizer-region model executes `finally` on `if`, `when`, `while`, and `do ... while`; routes normal completion, `when` yield, return, break, and crossed continue; acquires values before branch cleanup; keeps `given` locals alive through cleanup; runs nested regions inner-to-outer; skips the loop finalizer on same-loop continue; and preserves fatal panic as abort-without-cleanup. Shared MIR validation drives the interpreter, Cranelift, and LLVM; PHP preserves the same checked semantics. Stage 29 is next and must extend these regions rather than creating a second cleanup mechanism. Controlled timing is Pending Available Runner and non-blocking.
- **Stage 29 — Checked errors end-to-end — In Progress.** Decision 0119 settles the complete model. **Slice 1 — Complete:** dedicated `try`/`catch`/`throw`/`throws` grammar; backend-independent AST/HIR; compiler-known explicit `Error` conformance with readonly external `string $message`; source-ordered semantic effect sets; exact concrete and `Error` catch coverage; catch-or-declare checking; throw ownership; constructor/property/static/constant/destructor/finally effects; diagnostics; and tooling. **Slice 2 — Complete:** the two-word carrier, static descriptors, hidden first-origin storage, checked-result ABI, explicit checked MIR, propagation, exact and catch-all dispatch, optional bindings, rethrow, Stage 28a finalizer reuse, failed-construction cleanup, ordinary Error values in supported aggregates and `mixed`, shared MIR validation, and interpreter/Cranelift/LLVM/PHP transport parity. B2901 is historical and unreachable for valid handled programs. **Corrective Beat: Native Collection Property Initializers — Complete:** every concrete specialized instance-property and payload-enum storage type is interned before any callable lowers, so contextual empty and nonempty collection initializers execute for readonly and writable properties under implicit and explicit constructors. Durable coverage includes concrete and generic class elements, nested lists, typed arrays, dictionaries, capability-valid set/sorted families, deque, independent per-instance storage, and reverse destruction across the interpreter, Cranelift, LLVM, and the PHP-compatible subset. Unsupported combinations still stop at their semantic capability diagnostic. This closes the former N1101 ordering defect only; E0472 owned-property transfer remains a separate language and implementation beat. **Slice 3 — Next:** the record-0075 I/O migration, canonical `Doria\Std\Io` Error types, R1000 shared runtime outcome, status 70, and process/tooling closure; until then B2902 stops a checked Error escaping `main` before artifact emission. Checked errors clean up but never roll back side effects; panic remains fatal status 101 and cleanup-free; the ordinary closed-standard-stream status-0 rule remains permanent and never throws. Stage 30 stays blocked until Stage 29 completes.

### Phase F — Multi-file, namespaces, Baton (Stages 30–33)
- **Pre-Stage-30 Grammar Slice — Closure accepted syntax.** Scheduled immediately after Stage 29 and before Stage 30 under decision 0120 and the §0 two-clocks rule. Owns `fn` and anonymous-function expression grammar, explicitly typed parameters, arrow return inference syntax, function-type syntax, explicit `with` lists and readonly/writable/taking capture-mode syntax, source-preserving AST nodes, recovery, and accepted-syntax tests. Successfully parsed closures stop at one stage-named Stage 30 unsupported-feature boundary. This slice does not perform free-variable discovery, capture validation, HIR/MIR lowering, ownership analysis, environment construction, collection algorithms, or backend execution. Documentation snippets remain outside checked `.doria` examples until this slice lands.
- **Stage 30 — Closures.** Decisions 0120, D10, D20, and 0119: consumes the pre-Stage-30 source-preserving closure AST; owns missing-capture diagnostics and fixes; capture validation, ownership, environment representation, function types, calls, checked-effect integration, and borrow-bound escape checking; Decision 0100's accepted `List<T>` `map`/`filter`/`reduce`; interpreter, Cranelift, LLVM, PHP compatibility, language-server semantic integration, and website/playground activation. PHP lowering follows Doria's written captures, not PHP defaults. Async/cross-task capture remains in the later structured-concurrency stage; general erased interface interaction remains Stage 35; `$this`, closure effect inference/annotation, and implementation slicing remain bounded Stage 30 design work. **Stage 30 remains blocked until Stage 29 and the pre-Stage-30 grammar slice complete; it is not implemented.**

  Stage 30 acceptance criteria:

  1. A no-capture arrow closure requires no `with`.
  2. A no-capture anonymous closure requires no `with`.
  3. An arrow using an outer local requires `with`.
  4. An anonymous function using an outer local requires `with`.
  5. Copy locals are not captured implicitly.
  6. Move locals are not captured implicitly.
  7. Readonly captures use `with ($value)`.
  8. Writable captures use `with (writable $value)`.
  9. Taking captures use `with (take $value)`.
  10. Arrow-to-block refactoring preserves the capture list and ownership modes.
  11. A missing capture receives a capture-specific diagnostic.
  12. A safe structured fix inserts or extends the required capture clause.
  13. A closure without captures is never forced to write `with ()`.
  14. Borrowed captures cannot outlive their owners.
  15. Taking captures move the source and make it unavailable afterward.
  16. Closure callable types preserve Stage 29's checked-effect sets and subset law.
  17. `List<T>` `map`, `filter`, and `reduce` use this same closure model.
  18. PHP lowering follows Doria capture semantics rather than PHP defaults.
  19. Capture discovery uses no hidden runtime reflection.
  20. No automatic-capture exception exists for Copy values.
- **Stage 31 — Namespaces and package compilation graph.** **Slice 1:** decisions 0028 and 0117 name syntax and resolution, absolute qualified names, one resolution chain, group `use`, prelude, `include` grammar, package identity in compiler-facing inputs, and LSP multi-file symbol identity. **Slice 2:** versioned JSON build plans, stable source/package identity, multi-file indexing, direct-dependency visibility, package-wide `internal`, hybrid strict layout, include-once, declaration-only non-entry files, duplicate symbols across packages, cross-file diagnostics, and compiler incremental inputs. Qualified names resolve in every type position: `extends`, `implements`, parameters, returns, properties, locals, `new`, statics, constants, and `is`. AC: multi-package project build; every active file checked; complete duplicate-symbol provenance; absolute-on-`\`, group-use, prelude-shadowing, intrinsic-redeclaration, include/autoload deduplication, package-visibility, and accepted-syntax fixtures; coordinated LSP coverage.
...
- **Stage 33 — Baton package and dependency workflow.** Decision 0118. **Slice 1:** schema 2 parsing with exact schema 1 compatibility, autoload/autoload-dev discovery, main/development/generated scopes, binary/library targets, deterministic inventory, and single-package build plans. **Slice 2:** path and Git dependencies, SemVer validation, one-version conflicts, deterministic JSON `Baton.lock`, install/update/fetch/tree/why/add/remove, global content-addressed cache, and offline resolution. **Slice 3:** workspaces, development dependencies, graph commands, incremental inventory, `baton test`, explicit processor registration, generated-source orchestration, and Phase F closure. Stage 32 supplies typed metadata and the processor protocol; it does not own automatic package orchestration. AC: `baton new game && baton test` green; locked offline workspace build; direct-dependency visibility and conflict diagnostics; no implicit build scripts.
