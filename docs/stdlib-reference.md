...
Owned move types with a growable/fixed distinction. The complete family and naming are settled in **decision 0092**; the method surface originates in **decision 0100** and is completed by **decision 0113**. Decision 0113 Slices 1-3 are implemented: the renamed/widened membership surface, receiver-aware diagnostics, value search/removal, and set endpoints execute. Slice 4 `clear()` is accepted but not yet executable and receives a precise pending diagnostic before MIR. Closure methods wait for Stage 30. A bare name is the default (hash / insertion-ordered) collection; the `Sorted` prefix is the comparison-ordered variant. Cross-cutting rules (0100): reads are `readonly`, mutators `writable`; ingestion moves values in; `$l[i]`/`$d[k]` reads **assert presence and panic** on absence while the `?T` methods are the safe path; removal hands the owned element back; mutators return `void` (userland fluent APIs stay a decision-0088 capability).
...
- **`List<T>`** — the everyday growable sequence and default workhorse: `add`, `insertAt`, `removeAt` (returns the owned element), `pop` (`?T`), `contains`, `indexOf(T): ?int` (first equal position, `null` when absent), writable `remove(T): bool` (first equal element only), `first`/`last` (`?T` properties), `count`, and `isEmpty` (properties). Writable `clear(): void` remains accepted pending Slice 4. `map`/`filter`/`reduce` land with closures in Stage 30.
- **`Dictionary<K, V>` / `SortedDictionary<K, V>`** — `get` returning `?V`, `set`, `remove` returning `?V`, `containsKey` (key membership), executable O(n) `containsValue(V): bool` (value membership), `keys`/`values` (`foreach`-only projections, not storable in v1.0), and `count`/`isEmpty` (properties). Writable `clear(): void` remains accepted pending Slice 4. `Dictionary` iterates in insertion order, `SortedDictionary` by ascending `Comparable` key. Keys are readonly; values may be writable through the main iteration form.
- **`Set<T>` / `SortedSet<T>`** — `::from`, `add` (`bool`), `remove` (`bool`), `contains`, `union`, `intersect`, `difference`, executable readonly `first: ?T` / `last: ?T` properties, and `count`/`isEmpty` (properties). Writable `clear(): void` remains accepted pending Slice 4. `Set` endpoints follow insertion order; `SortedSet` endpoints are the ascending minimum and maximum. All endpoints are O(1), return `null` when empty, and borrow the retained element. Iteration is readonly: replacement is remove plus add. Existing-source construction and algebra preserve every input and require `Copy` or, once Stage 35 lands, `Cloneable`.
