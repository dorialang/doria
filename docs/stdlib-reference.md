...
- **`Doria\Std\Io`** — the scheduled Stage 36a byte-stream and standard-I/O foundation: capability-based readable/writable/duplex/seekable/flushable behavior; explicit EOF, would-block, and partial-progress results; first-class standard streams over the existing intrinsic runtime substrate; file handles with typed open modes; blocking control and portable readiness; buffering, UTF-8 text/line adapters, streaming copy, and process-pipe integration. Stage 29 supplies checked errors but does not implement this layer. Exact interface, result, mode, adapter, file, readiness, and standard-stream names remain pending Andrew's review and the unauthored **Stream, Readiness, Standard I/O, And Blocking-Mode Model** decision. The current text/binary free functions are *language intrinsics*, not proof that this module exists. *(Capability inventory: `docs/notes/io-surface-audit.md`; Stage 36a scheduled, public surface pending review.)*
...
- **`Doria\Std\Process`** — process facts (exit code, process id, executable path) and, after review, owned child processes with typed stdin/stdout/stderr pipes, half-close, wait/termination/status, concurrent drainage, output limits, and timeout/readiness integration. The pipe capability depends on Stage 36a and is not currently executable. Command-line arguments arrive through `main(List<string> $args)` (decision 0099), not here.
...
- **`Doria\Std\Net`** — networking, TCP first. Stage 44 builds its client/listener/socket and HTTP maturation on Stage 36a's duplex byte streams, readiness, timeouts, partial writes, and async integration; socket streams do not define a second I/O contract. TLS is a typed network-adapter boundary whose implementation remains Stage 44 review work. *(Surface TBD.)*
...
- **`Doria\Std\Term`** — the portable terminal layer for product 5, surfaced through the `Console` static facade. It reuses Stage 36a's standard-stream views, readiness, blocking substrate, timeouts, and platform-device abstraction; generic stream readiness is not reimplemented here. See "The terminal layer" in plan §9 and the Console/terminal decision. Surface (planned):
