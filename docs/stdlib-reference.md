...
Every primitive has a companion, and the set is a **complete, symmetric matrix** — not a per-type ad-hoc collection (decision 0104). The uniform v1.0 baseline: `parse(string): ?T` on every scalar companion (int family, float family, `bool`); `String` is the exclusive owner of accepted string-specific operation vocabulary instead of `parse` because `string` is the parse *domain* (0103); display is uniform through the display path, so no companion has `toString`. Absences are deliberate — a member not meaningful for a primitive is a documented N/A, one not in v1.0 is a named v1.0+ furnishing (`MIN`/`MAX`, `Bool::toInt`). Decision 0103's current operation list remains a seed pending the completeness review; the companion-ownership matrix, not that provisional inventory, is what is complete. Details: decisions 0013/0016 (numeric types), 0042 (conversions), 0095 (`pow`), 0096 (interface conformance), 0103 (canonical String API), 0104 (completeness); §4.6 (strings).
...
- **`String`** — intrinsic properties/views are `$s->length` (Unicode grapheme clusters), `$s->byteLength` (UTF-8 bytes), `$s->isEmpty`, `$s->bytes` (copy in v1.0), `$s->graphemes`, and `$s->codePoints`. Every accepted string-specific operation belongs to the `String` companion (decision 0103); its current seed lists trimming/casing (`trim`, `trimStart`, `trimEnd`, `lower`, `upper`), predicates (`contains`, `startsWith`, `endsWith`, `equalsIgnoreCase`), grapheme-indexed search (`indexOf`, `lastIndexOf`), `replace`, `split`/`join`, `slice`, `repeat`, `padStart`/`padEnd`, UTF-8-validating `fromBytes`, and the `Ordering`-returning `compare`/`compareIgnoreCase` family once `Ordering` is executable. The completeness review must settle the exhaustive v1 inventory before the Minimum String Runtime Surface selects its first implemented subset. There is no public `str_*` family, `$s->chars`, integer string indexing, or duplicate instance-method spelling.
