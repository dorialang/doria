...
- **`PriorityQueue<T>`** — `push`, `pop` (`?T`), `peek` (`?T`), and `contains`, min-first by `Comparable`; duplicates are allowed, equal elements have no stable tie order, and there is no `foreach` (drain via `pop`). **`Deque<T>`** — `pushFront`/`pushBack`/`popFront`/`popBack`/`peekFront`/`peekBack`/`contains`, readonly or writable `foreach` front-to-back; subsumes FIFO/LIFO, so there are no separate `Queue`/`Stack` types.
