...
- **`Dictionary<K, V>` / `SortedDictionary<K, V>`** — `get` returning `?V`, `set`, `remove` returning `?V`, `has` (key membership), `keys`/`values` (`foreach`-only projections, not storable in v1.0); `Dictionary` iterates in insertion order, `SortedDictionary` by ascending `Comparable` key. Keys are readonly; values may be writable through the main iteration form.
