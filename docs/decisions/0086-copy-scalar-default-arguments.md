...
Provenance note. This record was briefly placed in the plan slot reserved for the
namespace-model subject, then renumbered. The namespace-model subject and its
reference from decision 0074 both predate this record, so the original placement
was a catalogue collision rather than a durable citation. Per the numbering rule,
unauthored subjects have no number; a decision takes its real number only when its
file is authored. This record is 0086.
