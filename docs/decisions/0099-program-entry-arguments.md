...
- **An argument that is not valid UTF-8 panics** (amended post-acceptance, at implementation). `string` is defined as immutable UTF-8 and that invariant is load-bearing for the whole language, so a malformed argument must not enter the program as a `string`. This matches `read_file`, which validates before constructing a `string` rather than admitting invalid bytes, and it keeps the failure at the process boundary where it is diagnosable instead of surfacing as corrupt text somewhere downstream. Lossy replacement was rejected because it silently changes the user's input; a `?string` element or a `Bytes`-typed argument list was rejected because it would burden every correct CLI program with a decoding step to serve a case that is a mistake in practice. A future record may add a byte-oriented accessor through `Doria\Std\Process` for programs that genuinely need raw argument bytes; that is an addition, not a change to this contract.
- The argument list is **owned by the entry glue and borrowed by `main`** for the duration of the call, so `$args` is an ordinary readonly parameter: `writable` and `take` are rejected. The glue releases the list after `main` returns, which is why the callee carries no destruction obligation.
