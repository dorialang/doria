...
Doria accepts typed equality, bool-only boolean operators, integer bitwise operators, and `given` predicate block rules as language design direction. This decision records semantics only. It does not implement lexer, parser, semantic checker, Doria IR, PHP backend, native backend, `Bool` helper APIs, `given`, `finally`, or `when`.
