...
| Constructor-rooted writable child paths | Covered | Covered | Covered | Covered | `main_constructor_owned_property_writes.doria` proves deep property mutation through an initializer-initialized writable path without making direct `$this` writable. |
| Owned property initialization and replacement | Covered | Covered | Covered | Covered | Explicit write-kind MIR distinguishes initialization, replacement, and merged constructor state; the durable constructor fixtures prove exact acquire-before-drop output. |
...
| Native Testing Foundation Slices 1 through 3 | Covered | Covered | Covered | Covered | Behavioral declarations, scalar/string/collection/Error expectations, exact ownership and cleanup, bounded failure presentations and differences, and strict DORIAO4 matcher facts share one validated HIR/MIR contract; metadata schemas 1 through 3 remain exact. |
| Stage 34 single class inheritance | Covered | Covered | Covered | Covered | Thirty durable fixtures cover closed/open and three-level hierarchies, generic parents and virtual specialization, parent constructors/defaults/methods/statics, parent-prefix properties, root-first construction, derived-first destruction, lifecycle-phase dispatch, direct and virtual checked/ambient/TestAssertion effects, nullable/mixed/shared/collection carriers, allocation-free upcasts, `is`/match narrowing, Error catches and `toThrow`, dynamic drop, and exact-call LLVM devirtualization. Shared MIR validation independently requires valid hierarchy identities, carriers, slots, dominating refinement proofs, lifecycle phases, and dynamic-drop contracts. |
...
| Direct `parent::` and trait-local `self::` grammar | Covered | Covered | Covered | Covered | Parent constructor, instance, static, constant, and static-property forms resolve to the immediate parent and bypass virtual dispatch. Trait-local `self::` remains accepted syntax with a Stage 35 semantic diagnostic; companion LSP coverage is coordinated in `dorialang/doria-language-server`. |
...
| Checked I/O failures | Covered | Covered | Covered | Covered | Missing files, invalid paths, invalid UTF-8, text/binary/device failures, and deterministic messages use canonical Doria Error types. P1401-P1407 are historical; allocation panic remains separate. |
...
descriptor identity. An Error escaping `main` reports R1000 after cleanup and
exits 70; a successful return of 70 emits no R1000. Existing nonthrowing parity,
fatal status 101, and ordinary broken-pipe status 0 remain unchanged.
