...
| E0641 closure target boundary | Ownership-valid closure values execute only through the debug interpreter | Native and PHP closure execution are separate staged backends | Accepted But Not Implemented for native/PHP; implemented for debug | Decision 0121 | Native Stage 30e; PHP Stage 30f | Yes only as a target-specific `UnsupportedDevelopmentSurface`; it is invalid in checking, IDE analysis, HIR/MIR, and debug execution |
