...
| E0641 closure target boundary | Supported plain closures formerly stopped before PHP emission | PHP compatibility required explicit Doria capture and ownership lowering | Implemented for debug, Cranelift, LLVM, and the PHP-supported value surface; catalogue identity retained until Stage 30h | Decision 0121 | Stage 30h final audit | No valid supported plain closure route emits it; unrelated PHP capability diagnostics remain independent |
| `List<T>` higher-order methods | Compiler-known callback contracts formerly had no executable collection traversal | Closure ownership, exact effects, indirect calls, and partial-result cleanup had to exist first | Implemented for `map`, Copy-only `filter`, and writable-accumulator `reduce`; other collection families remain outside this contract | Decision 0121 | Stage 30h cross-repository audit | No valid supported List algorithm emits E0641; contract errors use compiler-owned E0664-E0668 identities |
