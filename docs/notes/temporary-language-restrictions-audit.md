...
| E0641 closure target boundary | Ownership-valid closure values execute through debug, Cranelift, and LLVM, but not PHP | PHP compatibility requires explicit Doria capture and ownership lowering | Accepted But Not Implemented for PHP; implemented for debug/native | Decision 0121 | PHP Stage 30f | Yes only as a PHP-target `UnsupportedDevelopmentSurface`; it is invalid in checking, IDE analysis, HIR/MIR, debug, and native execution |
