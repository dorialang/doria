...
| `Doria\Std\Test` collection/Error expectation members | Core `expect`, `fail`, `AssertionError`, and `TestAssertion` compiler/runtime behavior is implemented; collection matchers and `toThrow` remain unavailable | Collection/Error expectations, bounded differences, and final Baton/tooling closure belong to Slice 3 | Partially Implemented | Decision 0129 | Native Testing Foundation Slice 3 | Yes when one structured Slice-3 unsupported-development diagnostic is emitted; E0710 remains historical and reserved |
