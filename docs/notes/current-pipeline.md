...
- Decision 0121 accepts the complete Stage 30 closure authority. Stages 30a and 30b are complete: structural function types have canonical semantic identity; closure bodies and explicit captures are checked by stable binding identity; invocation modes and checked effects are inferred; nullable callable narrowing, callable-value calls, and callable properties are checked. E0641 is narrowed to valid execution routes. Stage 30c Ownership, Lifetime, And Escape is next; Stage 30 remains in progress and not complete. No capture acquisition, lifetime/escape enforcement, HIR/MIR closure representation, backend execution, or collection algorithm was added.
