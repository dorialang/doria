...
- Stage 29 is in progress under accepted decision 0119. Slices 1 and 2 are complete. The compiler checks explicit Error conformance, source-ordered effects, catch coverage, ownership, construction, and scope rules, then executes handled checked errors through one two-word carrier, static descriptor/origin tables, explicit checked MIR, status/out-slot ABI, exact/catch-all dispatch, rethrow, structured finalizer routing, and failed-construction cleanup. The interpreter, Cranelift, LLVM, and PHP compatibility transport agree on supported programs; B2901 has no valid route. Slice 3 is next and owns canonical I/O errors/signature migration, R1000, status 70, and process reporting for an Error escaping `main`, which remains gated by B2902. Stage 30 remains blocked until Stage 29 completes.
