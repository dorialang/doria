...
- Trait composition and trait-local member execution until Stage 35; their accepted grammar is already represented.
- Growable/slice/search `Bytes` members until a future method-surface decision.
- Writable match and writable payload patterns are rejected for Doria v1; interface `is` remains deferred until Stage 35.
- Collection/interface `is`, plus boxing collections, typed arrays, or `Bytes` into `mixed`, until their authored stages. Class subtype `is` is implemented by Stage 34.
