...
- Stage 20a/20b const-evaluable defaults are complete for Copy scalars and readonly strings across free functions, instance methods, static methods, and constructors through one caller-side MIR splice. Writable Copy scalars remain supported; `writable string`, `take string`, and other move/`take` defaults retain explicit temporary diagnostics.
...
- Stage 21: non-lexical borrowing and full definite initialization.
