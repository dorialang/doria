...
- Decision 0121 accepts the complete Stage 30 closure authority. Stages 30a through 30c are complete: structural function types have canonical semantic identity; closure bodies and explicit captures are checked by stable binding identity; invocation modes and checked effects are inferred; nullable callable narrowing, callable-value calls, and callable properties are checked; captures acquire at creation in authored order; every function value is Move; readonly/writable leases end at last use; once calls consume path-sensitively; and escape, storage, callback retention, return roots, nested provenance, and reverse logical release plans are enforced. E0641 remains only as the HIR/MIR/runtime execution boundary. Stage 30d Closure HIR/MIR And Interpreter Oracle is next; Stage 30 remains in progress and not complete. No closure HIR/MIR representation, backend execution, or collection algorithm was added.
