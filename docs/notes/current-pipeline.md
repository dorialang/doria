...
- Stage 24 generic free functions, instance methods, and static methods are complete on the current branch. Type arguments are inferred through the existing positional/named argument binding, with a typed declaration supplying expected-result context when arguments alone cannot resolve a parameter. MIR monomorphization emits one reachable specialization per distinct concrete argument set and deduplicates repeated uses; its kinded specialization key leaves compile-time value parameters as an additive future extension. `<T implements A, B>` is accepted and carried, while constraint enforcement remains explicitly pending until Stage 35. Generic classes remain Stage 25, and the generics decision is still unauthored.
...
- Stage 25: generic classes, building on Stage 24's function/method inference and MIR specialization machinery. The generics decision remains unauthored and is a merge prerequisite for this generics work.
