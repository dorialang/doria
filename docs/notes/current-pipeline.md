...
- Decision 0121 accepts the complete Stage 30 closure authority. Stages 30a through 30d are complete: structural function types have canonical semantic and MIR identity; closure bodies and explicit captures are checked by stable binding identity; captures acquire at creation in authored order; every function value is Move; readonly/writable leases end at last use; once calls consume path-sensitively; and escape, storage, callback retention, return roots, nested provenance, and reverse logical release plans are enforced. Explicit closure/callable-call HIR, static descriptors, logical environment layouts, synthetic functions, indirect and checked indirect MIR, shared validation, stable interpreter places, and exact function-value cleanup now execute through the debug interpreter. The logical carrier is two words, no-capture closures allocate no environment, and ordinary closures use no reference counting. E0641 remains only at the native Stage 30e and PHP Stage 30f target boundaries. Stage 30e Native Execution is next; Stage 30 remains in progress and not complete. No native/PHP closure execution or collection algorithm was added.
