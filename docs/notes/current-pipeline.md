...
- Decision 0120 accepts one explicit closure-capture model for Stage 30: arrows and anonymous block functions must list enclosing local dependencies with `with`; Copy and Move values have no implicit exception; no-capture closures omit `with`. This is authority only. Stage 30 remains blocked and unimplemented, `$this` capture remains a bounded Stage 30 question, and Decision 0119 remains the checked callable-effect foundation.
