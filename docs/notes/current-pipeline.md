...
- Decision 0120 accepts one explicit closure-capture model: arrows and anonymous block functions must list enclosing local dependencies with `with`; Copy and Move values have no implicit exception; no-capture closures omit `with`. This is authority only. The bounded pre-Stage-30 grammar slice is next so accepted syntax reaches lexer/parser/AST and one Stage 30 boundary before semantic implementation. Stage 30 remains blocked and unimplemented, `$this` capture remains a bounded Stage 30 question, and Decision 0119 remains the checked callable-effect foundation.
- Phase F namespace, autoload, package, and dependency authority is accepted under decisions 0117 and 0118. The public source-discovery term is `autoload`, discovery occurs at compile time, hybrid layout is strict, package `internal` is package-wide, and Baton passes a versioned JSON build plan rather than making `doriac` parse `Baton.toml`. Manifest schema 2, direct-only dependencies, path/Git resolution, deterministic JSON `Baton.lock`, workspaces, processors, a content-addressed cache, and offline behavior are fixed for their scheduled stages. The PHP Baton bootstrap still executes schema 1 only. Stage 31 is scheduled in two slices and not implemented; Stage 33 is scheduled in three slices and not implemented. The pre-Stage-30 closure grammar slice is next.
