...
- Decision 0121 accepts the complete Stage 30 closure authority. Stages 30a through 30f are complete: structural function types have canonical semantic and MIR identity; closure bodies and explicit captures are checked by stable binding identity; captures acquire at creation in authored order; every function value is Move; readonly/writable leases end at last use; once calls consume path-sensitively; and escape, storage, callback retention, return roots, nested provenance, and reverse logical release plans are enforced. Explicit closure/callable-call HIR, static descriptors, concrete native environment layouts, synthetic functions, indirect and checked indirect MIR, shared validation, stable places, and exact function-value cleanup execute through the debug interpreter, Cranelift fast profile, LLVM release profile, and PHP compatibility for the supported value surface. The native carrier is two words; no-capture closures allocate no environment; proven nonescaping native environments use stack storage; escaping native environments use one heap allocation; ordinary closures use no reference counting. PHP consumes semantic and validated MIR plans and emits compiler-owned carriers, environments, and BindingId-backed cells rather than automatic host capture. E0641 is unreachable for supported plain closure execution and remains catalogued until Stage 30h. Stage 30g List Algorithms is next; Stage 30 remains in progress and not complete.
