...
- Interface values, erased calls, conversions/tests, and shared-interface execution until Stage 35 Slice 2. Declarations, requirement graphs, and trait-free nominal conformance are checked in Slice 1.
...
- Writable match and writable payload patterns are rejected for Doria v1; interface `is` execution remains deferred until Stage 35 Slice 2.
- Collection/interface `is`, plus boxing collections, typed arrays, or `Bytes` into `mixed`, until their authored implementation slices. Class subtype `is` is implemented by Stage 34.
