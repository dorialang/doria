...
- `SharedReference<T>`/`WeakReference<T>`/`WritableSharedReference<T>` are rescheduled to Stage 25a, after nullable/narrowing and generic classes provide the machinery their separately unauthored API depends on.
...
- Stage 25a: opt-in shared ownership, after its shared-ownership decision is authored against the landed nullable, narrowing, and generic-class machinery.
