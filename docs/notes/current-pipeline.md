...
- Native Testing Foundation Slices 1 and 2 are complete under Decision 0129. Exact compiler-known `Doria\Std\Test` `describe`/`it`/`test` declarations are restricted to active development source, preserve const descriptions and nested suite/test facts, and elaborate test bodies into deterministic ordinary generated callables without runtime registration. Strict metadata schema 3 unifies behavioral declarations with `#[Test]` while schemas 1 and 2 remain exact. Slice 2 implements `expect`/`fail`, `AssertionError`, the separate automatic `TestAssertion` effect, structured DORIAO4 assertion outcomes, cleanup, interpreter/Cranelift/LLVM/PHP parity, Baton regression integration, and compiler-fact-based tooling coordination. Slice 3 is next and owns collection/Error expectations and final reporting/tooling closure. The Native Testing Foundation remains in progress and Stage 34 remains blocked until it completes.
