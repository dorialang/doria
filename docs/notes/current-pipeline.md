...
- Native Testing Foundation Slice 1 is complete under Decision 0129. Exact compiler-known `Doria\Std\Test` `describe`/`it`/`test` declarations are restricted to active development source, preserve const descriptions and nested suite/test facts, and elaborate test bodies into deterministic ordinary generated callables without runtime registration. Strict metadata schema 3 unifies behavioral declarations with `#[Test]` while schemas 1 and 2 remain exact. Slice 2's compiler/runtime portion now implements `expect`/`fail`, `AssertionError`, the separate automatic `TestAssertion` effect, structured DORIAO4 assertion outcomes, cleanup, and interpreter/Cranelift/LLVM/PHP parity. Baton regression integration and the official language-server pin/fact coordination remain pending. Slice 3 owns collection/Error expectations and final reporting/tooling closure. The Native Testing Foundation remains in progress and Stage 34 remains blocked until it completes.
