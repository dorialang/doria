...
- Stage 29 is complete under accepted and implemented decision 0119. Slices 1 and 2 provide checked syntax/effects, the two-word carrier, descriptors/origins, checked MIR/ABI, catch dispatch, propagation, finalizer routing, and failed-construction cleanup. The native collection property initializer corrective beat remains closed without absorbing E0472. Slice 3 adds the six exact compiler-known `Doria\Std\Io` identities, one authoritative built-in effect table, deterministic checked I/O across the interpreter, Cranelift, LLVM, and PHP, and R1000/status-70 reporting after cleanup for an Error escaping `main`. The inferred-main corrective beat is complete: reusable callables still declare escaping checked effects, while a clause-free selected entrypoint infers its exact uncovered source-ordered set and carries it through source calls, HIR/MIR, ABI selection, and all backends. Explicit `main throws` remains accepted. B2901 and B2902 have no valid route; P1401 through P1407 are historical for ordinary I/O. Allocation panics, fatal status 101, and ordinary closed-standard-pipe status 0 remain distinct.
- Decision 0120 accepts one explicit closure-capture model: arrows and anonymous block functions must list enclosing local dependencies with `with`; Copy and Move values have no implicit exception; no-capture closures omit `with`. The bounded pre-Stage-30 grammar slice is complete: `fn`, anonymous block closures, `function(T): R` type syntax, capture modes, source-preserving AST, recovery, and one catalogued `E0641` boundary are implemented.
- Decision 0121 accepts the complete Stage 30 closure authority. Structural Move-only function values distinguish readonly/writable/once invocation, preserve parameter ownership and checked effects, invoke callable-valued expressions positionally, capture `$this` explicitly, validate captures by binding identity, acquire captures at creation, use a lean two-word carrier, and add closure algorithms only to `List<T>`. Those algorithms borrow readonly callbacks readonly and writable callbacks exclusively, preserving the invocation capability instead of erasing it at the function-value parameter. The former [Stage 30 Closure Authority Proposal](stage30-closure-authority-proposal.md) is historical supporting material. Stage 30a Callable Grammar Completion is next; Stage 30 remains not implemented and E0641 remains active.
- Phase F namespace, autoload, package, and dependency authority is accepted under decisions 0117 and 0118. The public source-discovery term is `autoload`, discovery occurs at compile time, hybrid layout is strict, package `internal` is package-wide, and Baton passes a versioned JSON build plan rather than making `doriac` parse `Baton.toml`. Manifest schema 2, direct-only dependencies, path/Git resolution, deterministic JSON `Baton.lock`, workspaces, processors, a content-addressed cache, and offline behavior are fixed for their scheduled stages. The PHP Baton bootstrap still executes schema 1 only. Stage 31 is scheduled in two slices and not implemented; Stage 33 is scheduled in three slices and not implemented. Stage 30a Callable Grammar Completion is next.
