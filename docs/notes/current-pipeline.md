...
- Decision 0120 accepts one explicit closure-capture model: arrows and anonymous block functions must list enclosing local dependencies with `with`; Copy and Move values have no implicit exception; no-capture closures omit `with`. The bounded pre-Stage-30 grammar slice is complete: `fn`, anonymous block closures, `function(T): R` type syntax, capture modes, source-preserving AST, recovery, and one catalogued `E0641` boundary are implemented. Stage 30 is next and unimplemented; `$this` capture remains a bounded Stage 30 question, and Decision 0119 remains the checked callable-effect foundation.
- [Stage 30 Closure Authority Proposal](stage30-closure-authority-proposal.md) — In Review. This supporting proposal does not change accepted semantics: Stage 30 remains not implemented, Decision 0120 remains the current capture authority, and E0641 remains the compiler boundary.
- Phase F namespace, autoload, package, and dependency authority is accepted under decisions 0117 and 0118. The public source-discovery term is `autoload`, discovery occurs at compile time, hybrid layout is strict, package `internal` is package-wide, and Baton passes a versioned JSON build plan rather than making `doriac` parse `Baton.toml`. Manifest schema 2, direct-only dependencies, path/Git resolution, deterministic JSON `Baton.lock`, workspaces, processors, a content-addressed cache, and offline behavior are fixed for their scheduled stages. The PHP Baton bootstrap still executes schema 1 only. Stage 31 is scheduled in two slices and not implemented; Stage 33 is scheduled in three slices and not implemented. Stage 30 closures are next.
