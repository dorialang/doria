...
- Stage 28a is in progress under accepted decision 0116. Slice 1 executes value-returning `when`, `given` prelude gates on `if`, `when`, and `while`, and base `do ... while` through one validated MIR CFG across the interpreter, Cranelift, LLVM, and PHP compatibility backend. `given` setup runs once; predicates short-circuit in source order; `given ... while` reevaluates them before every condition check, including `continue`. The parser and HIR preserve `finally` on `if`, `when`, `while`, and `do ... while`, but one E0611 Slice 2 diagnostic stops it before MIR. Slice 2 is next; Stage 29 is blocked until the accepted finalizer model executes.
