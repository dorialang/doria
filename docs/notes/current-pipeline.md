...
- Decision 0121 accepts the complete Stage 30 closure authority. Stages 30a through 30h and Stage 30 are complete: structural function types have canonical semantic and MIR identity; closure bodies and explicit captures are checked by stable binding identity; captures acquire at creation in authored order; every function value is Move; readonly/writable leases end at last use; once calls consume path-sensitively; and escape, storage, callback retention, return roots, nested provenance, reverse logical release plans, and function values in `mixed` are enforced. Explicit closure/callable-call HIR, static descriptors, concrete native environment layouts, synthetic functions, indirect and checked indirect MIR, shared validation, stable places, and exact function-value cleanup execute through the debug interpreter, Cranelift fast profile, LLVM release profile, and PHP compatibility for the supported value surface. Stage 30g adds compiler-known `List<T>::map`, Copy-only `filter`, and writable-accumulator `reduce`; one explicit semantic/HIR plan and validated MIR traversal preserve source-order callback execution, exact effects, readonly source borrowing, Move map results, Copy filtering, owned accumulators, and checked cleanup. PHP emits explicit ordered loops and no host higher-order array function. E0641 is historical and remains reserved.
...
- New core value operations, Cloneable widening, and public iteration until Stage 35 Slice 3; existing concrete contracts and built-in foreach remain executable.
