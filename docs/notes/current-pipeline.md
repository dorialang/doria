...
- Stage 27 Slice 1 is complete under decision 0114. Unit and `int`/`string`-backed enums are nominal inline Copy values with declaration-order case tags, exact equality, readonly backed `value`, nullable and `mixed` identity, const evaluation, supported collection placement, and native PHP compatibility lowering. The interpreter, Cranelift, and LLVM consume the same validated enum MIR and exact durable fixtures. Payload schemas and `match` expressions parse without syntax cascades but stop before MIR with their Slice 2 and Stage 28 diagnostics. Stage 27 remains in progress; payload execution is next.
...
- Stage 27 Slice 2 — Next.
- Stage 28 — Blocked Until Stage 27 Completes.
