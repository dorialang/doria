...
- Decision 0121 accepts the complete Stage 30 closure authority. Stages 30a through 30e are complete: structural function types have canonical semantic and MIR identity; closure bodies and explicit captures are checked by stable binding identity; captures acquire at creation in authored order; every function value is Move; readonly/writable leases end at last use; once calls consume path-sensitively; and escape, storage, callback retention, return roots, nested provenance, and reverse logical release plans are enforced. Explicit closure/callable-call HIR, static descriptors, concrete native environment layouts, synthetic functions, indirect and checked indirect MIR, shared validation, stable places, and exact function-value cleanup execute through the debug interpreter, Cranelift fast profile, and LLVM release profile. The native carrier is two words; no-capture closures allocate no environment; proven nonescaping environments use stack storage; escaping environments use one heap allocation; ordinary closures use no reference counting. E0641 remains only at the PHP Stage 30f target boundary. Stage 30f PHP Compatibility is next; Stage 30 remains in progress and not complete. No PHP closure execution or collection algorithm was added.
