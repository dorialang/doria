...
- Decision 0121 accepts the complete Stage 30 closure authority. Stage 30a Callable Grammar Completion is complete: the lexer, parser, and source AST preserve `once`, readonly/writable/once invocation modes, parameter ownership, function-type `throws`, parenthesized type grouping, and arbitrary postfix callable-value calls. Grouping disambiguates nested checked effects and is not a tuple. Stage 30b Semantic Function Types And Captures is next; Stage 30 remains in progress and not complete, and E0641 remains active. No closure HIR, MIR, runtime representation, backend execution, capture validation, or collection algorithm was added.
- Phase F namespace, autoload, package, and dependency authority is accepted under decisions 0117 and 0118. The public source-discovery term is `autoload`, discovery occurs at compile time, hybrid layout is strict, package `internal` is package-wide, and Baton passes a versioned JSON build plan rather than making `doriac` parse `Baton.toml`. Manifest schema 2, direct-only dependencies, path/Git resolution, deterministic JSON `Baton.lock`, workspaces, processors, a content-addressed cache, and offline behavior are fixed for their scheduled stages. The PHP Baton bootstrap still executes schema 1 only. Stage 31 is scheduled in two slices and not implemented; Stage 33 is scheduled in three slices and not implemented. Stage 30b Semantic Function Types And Captures is next.
