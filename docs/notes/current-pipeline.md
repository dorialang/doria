...
- Stage 29 is complete under accepted and implemented decision 0119. Slices 1 and 2 provide checked syntax/effects, the two-word carrier, descriptors/origins, checked MIR/ABI, catch dispatch, propagation, finalizer routing, and failed-construction cleanup. The native collection property initializer corrective beat remains closed without absorbing E0472. Slice 3 adds the six exact compiler-known `Doria\Std\Io` identities, one authoritative built-in effect table, deterministic checked I/O across the interpreter, Cranelift, LLVM, and PHP, and R1000/status-70 reporting after cleanup for an Error escaping `main`. B2901 and B2902 have no valid route; P1401 through P1407 are historical for ordinary I/O. Allocation panics, fatal status 101, and ordinary closed-standard-pipe status 0 remain distinct.
